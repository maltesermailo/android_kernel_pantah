...
sub find_text_symbol {
	...
	if ($symbol eq ".text" && $expr eq '+') {
		$offset = trim_zeros($offset);
		$symbol = $text_symbols->{$offset};

		if (!defined($symbol)) {
			die "$0: unknown symbol at .text+0x$offset";
		}
	}
...
sub read_symbols {
	open(my $fh, "\"$nm\" -n --defined-only \"$file\" 2>/dev/null |")
		or die "$0: ERROR: failed to execute \"$nm\": $!";

	while (<$fh>) {
		chomp;

		my ($addr, $symbol) = $_ =~ /^([a-f0-9]{16})\s[Tt]\s(.*)$/;

		if (defined($addr) && defined($symbol)) {
			$addr = trim_zeros($addr);
			$text_symbols->{$addr} = $symbol;
		}
	}
