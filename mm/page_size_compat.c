 ...
 * 10 bytes of __PAGE_SIZE-ed page. Check if this is the case and pace the magic on
...
int __fixup_swap_header(struct file *swap_file, struct address_space *mapping)
{
	...
	if (memcmp(magic, swap_header->magic.magic, 10))
		goto out; /* The mkswap tool may have hardcoded a 4096 page size */
	else
...
out:
	if (header_page && !IS_ERR(header_page)) {
		kunmap(header_page);
		put_page(header_page);
	}

	if (magic_page && !IS_ERR(magic_page)) {
		kunmap(magic_page);
		put_page(magic_page);
	}
