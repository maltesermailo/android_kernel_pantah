...
static int ashmem_compat_set_prot_mask(struct file *file, unsigned long prot)
{
	...
	/*
	 * memfds are always readable; there is no way to remove readability,
	 * nor is there a known usecase that requires it.
	 */
	prot |= PROT_READ;
	...
	/*
	 * Note: This does allow clearing PROT_EXEC, if necessary. This is only temporary
	 * while support for F_SEAL_FUTURE_EXEC lands.
	 */
...
{
	return memfd_filp_create(name, flags);
}
...
int ashmem_compat_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	/*
	 * Enforce file seals for shared mappings instead of all mappings to not break
	 * existing usecases that make private writable mappings on write-sealed files.
	 */
	if (vma->vm_flags & VM_SHARED) {
		...
		/*
		 * Clear matching VM_MAY* flags from the set of permissions that are blocked
		 * to prevent the region's permissions from changing with mprotect() later.
		 */
