...
/*
 * we need call trace_android_vh_add_page_to_lrulist in include/linux/mm_inline.h, but direct include
 * <trace/hooks/mm.h> bazel build will compile err.
 * so create an indirect call _trace_android_vh_add_page_to_lrulist and EXPORT_SYMBOL_GPL
 * in include/linux/mm_inline.h, we can declare symbol external, then call the indirect one.
 */
...
/*
 * we need call trace_android_vh_del_page_from_lrulist in include/linux/mm_inline.h, but direct include
 * <trace/hooks/mm.h> bazel build will compile err.
 * so create an indirect call _trace_android_vh__trace_android_vh_del_page_from_lrulist and EXPORT_SYMBOL_GPL
 * in include/linux/mm_inline.h, we can declare symbol external, then call the indirect one.
 */
...
/*
 * we need call trace_android_vh_update_page_mapcount in include/linux/rmap.h, but direct include
 * <trace/hooks/mm.h> bazel build will compile err.
 * so create an indirect call _trace_android_vh_update_page_mapcount and EXPORT_SYMBOL_GPL
 * in include/linux/rmap.h, we can declare symbol external, then call the indirect one.
 */
