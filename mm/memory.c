...
#if ALLOC_SPLIT_PTLOCKS
	/*
	 * The __pte_map_lock can still be using ->ptl, if spin_try_lock is
	 * tried after the above check which means use-after-free during
	 * unlock. Since the speculative decissions are executed under local
	 * interrupts disabled, synchronize it using the smp_call_*().
	 */
