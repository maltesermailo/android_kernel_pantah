...
again:
		...
		if (PageGCMAFree(page)) {
			/*
			 * Isolate the from the free list to prevent further
			 * allocation.
			 */
			ClearPageGCMAFree(page);
			list_del_init(&page->lru);
		...
		if (!get_page_unless_zero(page)) {
			...
			rcu_read_unlock();
			/*
			 * the page is in the middle of freeing but not reached
			 * to free list.
			 */
			goto again;
		}
...
static bool try_remove_inode(struct gcma_inode *inode)
{
	struct page *page;
	...
	return refcount_freeze(&inode->ref_count, 1);
}
