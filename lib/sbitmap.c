...
static spinlock_t *sbitmap_spinlock(struct sbitmap *sb, unsigned int index)
{
	...
	spinlock_t *const base_lock = (spinlock_t *)&sb->map[max_map_nr];

	if (index < 0 || index >= sb->map_nr) {
		pr_info("map_nr %d max %d index %d\n", sb->map_nr, max_map_nr,
			index);
		BUG();
	}
		      ...
		      bool alloc_hint)
{
	 ...
	 */
	BUILD_BUG_ON(sizeof(*sb->map) % sizeof(spinlock_t) != 0);
	const size_t map_size = sizeof(*sb->map) +
		sb->map_nr * (sizeof(*sb->map) + sizeof(spinlock_t));
