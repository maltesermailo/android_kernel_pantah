 ...
 *
 * This is a sample program which does some tests of the fips140 module:
 *
 *   - Call some ioctls on /dev/fips140 and print the results.
 *   - Use AF_ALG to try to call some crypto algorithms with invalid parameters,
 *     and print the results.
 *   - Use AF_ALG to dump some output from jitterentropy_rng.
 *
 * The purpose of this program is to allow the FIPS certification lab to test
 * some services of fips140.ko, which they are required to do.  This is a sample
 * program only, and it can be modified by the lab as needed.  This program must
 * be run as root.  The tests that use /dev/fips140 only work if the system has
 * loaded a build of fips140.ko that has evaluation testing support enabled.
 * The tests that use AF_ALG only work if the kernel has AF_ALG enabled, and
 * they will only test the fips140 module if it is actually loaded.
 *
 ...
 *
 * For advanced usage, try 'adb shell /data/local/tmp/fips140_lab_test --help'.
 */
...
static void test_version_service(void)
{
...
static void test_service_indicators(void)
{
	int i;
...
static const struct invalid_input_test {
...
} invalid_input_tests[] = {
	{
		...
		.alg_type = "skcipher",
		.alg_name = "xts(aes)",
		.key = (const uint8_t *)"0123456789ABCDEF0123456789ABCDEF",
		.key_size = 32,
		.setkey_error = EINVAL,
	}
};
...
static size_t jitter_dump_amount = 128;

static void test_jitterentropy(void)
{
	int alg_fd = get_alg_fd("rng", "jitterentropy_rng");
	int req_fd = get_req_fd(alg_fd, "jitterentropy_rng");
	size_t count = jitter_dump_amount;
	uint8_t buf[AF_ALG_MAX_RNG_REQUEST_SIZE];

	printf("Jitter RNG output: ");
	while (count) {
		ssize_t ret;

		memset(buf, 0, sizeof(buf));
		ret = read(req_fd, buf, MIN(count, sizeof(buf)));
		if (ret < 0)
...
static const struct test {
	...
	{ "version_service", test_version_service },
	{ "service_indicators", test_service_indicators },
	{ "invalid_inputs", test_invalid_inputs },
	{ "jitterentropy", test_jitterentropy },
};
...
static void usage(void)
{
	...
	fprintf(stderr, "Usage: fips140_lab_test TEST_TO_RUN...\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Available tests: ");
	for (i = 0; i < ARRAY_SIZE(tests); i++) {
		fprintf(stderr, "%s%s", tests[i].name,
			(i < ARRAY_SIZE(tests) - 1 ? ", " : " "));
	}
	fprintf(stderr, "\n"
"By default, all tests are run.\n"
"\n"
"Options:\n"
"  --dump-amount=AMOUNT      For jitterentropy test, amount to dump in bytes.\n"
	);
}

static bool is_test_enabled(const char *name, int argc, char *argv[])
{
	int i;
...
int main(int argc, char *argv[])
{
	static const struct option longopts[] = {
		{ "dump-amount", required_argument, NULL, 'j' },
		{ NULL, 0, NULL, 0 },
	};
	int c;
	int i;
	...
	while ((c = getopt_long(argc, argv, "", longopts, NULL)) != -1) {
		switch (c) {
		case 'j':
			jitter_dump_amount = strtoul(optarg, NULL, 0);
			if (jitter_dump_amount == 0 ||
			    jitter_dump_amount == ULONG_MAX)
				die("invalid --dump-amount");
			break;
		default:
			usage();
			return 1;
	...
	for (i = 0; i < ARRAY_SIZE(tests); i++) {
		if (is_test_enabled(tests[i].name, argc, argv))
			tests[i].func();
	}
	return 0;
}
