					     ...
					     unsigned long nr_subpages)
{
	...
	for (i = 0; i < entries; i++) {
		...
		/*
		 * Zero the PFN since there is no guaranteee the PFNs are contiguous.
		 * Zero the flags - applicable flags are derived from the sub-entries,
		 *                  inapplicable flags are kept zeroed.
		 */
		if (do_div(index, nr_subpages) == 0)
			res_vec[i / nr_subpages] = make_pme(0, 0);

		res_vec[i / nr_subpages].pme |= src_vec[i].pme & (PM_SOFT_DIRTY|PM_MMAP_EXCLUSIVE|PM_FILE|PM_PRESENT);
	}
}
			    ...
			    size_t count, loff_t *ppos)
{
	...
	if (unlikely(nr_subpages > 1)) {
		...
		res = kmalloc_array(pm.len / nr_subpages, PM_ENTRY_BYTES, GFP_KERNEL|__GFP_ZERO);
		if (!res) {
			ret = -ENOMEM;
			kfree(pm.buffer);
			goto out_mm;
