...
const struct mem_range incfs_pseudo_file_names[] = {
        { .data = (u8 *)pending_reads_file_name, .len = ARRAY_SIZE(pending_reads_file_name) - 1},
	{ .data = (u8 *)log_file_name, .len = ARRAY_SIZE(log_file_name) - 1 },
        { .data = (u8 *)blocks_written_file_name, .len = ARRAY_SIZE(blocks_written_file_name) - 1}
};

const unsigned long incfs_pseudo_file_inodes[] = {
        INCFS_PENDING_READS_INODE, INCFS_LOG_INODE, INCFS_BLOCKS_WRITTEN_INODE
};

static const struct file_operations *const pseudo_file_operations[] = {
        &incfs_pending_reads_file_ops,
        &incfs_log_file_ops,
...
static bool is_pseudo_filename(struct mem_range name)
{
	...
	for (; i < ARRAY_SIZE(incfs_pseudo_file_names); ++i)
        	if (incfs_equal_ranges(incfs_pseudo_file_names[i], name)) return true;
        return false;
}
...
static bool get_pseudo_inode(int ino, struct inode *inode)
{
        ...
        for (; i < ARRAY_SIZE(incfs_pseudo_file_inodes); ++i)
                if (ino == incfs_pseudo_file_inodes[i]) break;
        if (i == ARRAY_SIZE(incfs_pseudo_file_inodes)) return false;
...
int dir_lookup_pseudo_files(struct super_block *sb, struct dentry *dentry)
{
	...
	int i = 0;
	for (; i < ARRAY_SIZE(incfs_pseudo_file_names); ++i)
        	if (incfs_equal_ranges(incfs_pseudo_file_names[i], name_range)) break;
        if (i == ARRAY_SIZE(incfs_pseudo_file_names)) return -ENOENT;
