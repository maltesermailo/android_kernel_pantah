...
static long ioctl_get_read_timeouts(struct mount_info *mi, void __user *arg)
{
	...
	if (!error) {
		for (i = 0; i < size / sizeof(*buffer); ++i) {
			...
			t->uid = le32_to_cpu(t->uid);
			t->min_time_ms = le32_to_cpu(t->min_time_ms);
			t->min_pending_time_ms =
				le32_to_cpu(t->min_pending_time_ms);
			t->max_pending_time_ms =
				le32_to_cpu(t->max_pending_time_ms);
		}
...
static long ioctl_set_read_timeouts(struct mount_info *mi, void __user *arg)
{
	...
	for (i = 0; i < size / sizeof(*buffer); ++i) {
		...
		t->uid = cpu_to_le32(t->uid);
		t->min_time_ms = cpu_to_le32(t->min_time_ms);
		t->min_pending_time_ms = cpu_to_le32(t->min_pending_time_ms);
		t->max_pending_time_ms = cpu_to_le32(t->max_pending_time_ms);
