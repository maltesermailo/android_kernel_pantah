...
static int usleep_interruptible(int us)
{
	int ms = us / 1000;

	if (ms) {
		int error = msleep_interruptible(ms);
...
static int wait_for_data_block(struct data_file *df, int block_index,
			       int min_time_us, int min_pending_time_us,
			       int max_pending_time_us,
			       struct data_file_block *res_block)
{
...
ssize_t incfs_read_data_file_block(struct mem_range dst, struct file *f,
			int index, int min_time_us,
			int min_pending_time_us, int max_pending_time_us,
