			      ...
			      struct incfs_sysfs_node *node)
{
	...
	for (d = file->f_path.dentry; d->d_parent != d; d = d->d_parent)
		len += strlen(d->d_name.name) + 1;

	filename = kzalloc(len + 1, GFP_NOFS);
	if (!filename)
		return -ENOMEM;

	for (d = file->f_path.dentry; d->d_parent != d; d = d->d_parent) {
		size_t leaf_len = strlen(d->d_name.name);

		len -= leaf_len;
		memcpy(filename + len, d->d_name.name, leaf_len);
		len -= 1;
		filename[len] = '/';
	}
	...
	old_filename = node->isn_le_filename;
	node->isn_le_filename = filename;
	node->isn_le_time = ktime_get_real();
	node->isn_le_page = index;
	node->isn_le_result = result;
...
out:
	if (mi->mi_sysfs_node) {
		...
		/* Ignore error from logging error ... */
		incfs_update_sysfs_error(f, index, result, mi->mi_sysfs_node);
	}
