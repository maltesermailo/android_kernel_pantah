 ...
 * This structure will only exist for verity-enabled files
 */
struct incfs_file_verity_signature {
	struct incfs_md_header vs_header;

	 /*
	  * The size of the signature. If a file has verity enabled but no
	  * signature, vs_size will be zero.
	  */
	__le32 vs_size;

	 /*
	  * Signature's offset in the backing file. If a file has verity
	  * enabled but no signature, vs_offset will be zero.
	  */
	__le64 vs_offset;
} __packed;
