...
struct incfs_file_verity_descriptor {
	struct incfs_md_header vd_header;

	__le32 vd_size; /* The size of the signature. */

	__le64 vd_offset; /* Signature's offset in the backing file */
} __packed;

/* In memory version of above */
struct incfs_df_verity_descriptor {
	u32 size;
	u64 offset;
};
...
struct metadata_handler {
	...
	union {
		...
		struct incfs_file_verity_descriptor verity_descriptor;
	} md_buffer;
	...
	int (*handle_status)(struct incfs_status *sig,
				 struct metadata_handler *handler);
	int (*handle_verity_descriptor)(struct incfs_file_verity_descriptor *d,
					struct metadata_handler *handler);
};
