...
static ssize_t get_user_signature_info_size(u8 __user *signature)
{
	u32 buffer;
	u32 size = 0;
	int i;

	if (copy_from_user(&buffer, signature, sizeof(buffer)))
		return -EFAULT;
	if (le32_to_cpu(buffer) != INCFS_SIGNATURE_VERSION)
		return -EFAULT;
	size += sizeof(buffer);

	for (i = 0; i < INCFS_SIGNATURE_SECTIONS; ++i) {
		if (copy_from_user(&buffer, signature + size, sizeof(buffer)))
			return -EFAULT;
		size += sizeof(buffer);
		/* Prevent someone from using overflow of the section sizes */
		if (le32_to_cpu(buffer) > INCFS_MAX_SIGNATURE_SIZE)
			return -EFAULT;
		size += le32_to_cpu(buffer);
	}
...
static struct mem_range incfs_copy_signature_info_from_user(u8 __user *original)
{
	...
	size = get_user_signature_info_size(original);
	if (size < 0)
		return range(ERR_PTR(size), 0);
		...
		u8 __user *user_signature_info)
{
	...
	if (user_signature_info) {
		raw_signature =
		    incfs_copy_signature_info_from_user(user_signature_info);
