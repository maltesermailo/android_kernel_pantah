			    ...
			    loff_t *ppos)
{
	...
	if (report_uid)
		reads_buf2 = (struct incfs_pending_read_info2 *)
			get_zeroed_page(GFP_NOFS);
	else
		reads_buf = (struct incfs_pending_read_info *)
			get_zeroed_page(GFP_NOFS);

	if (!reads_buf && !reads_buf2)
		return -ENOMEM;
	...
	if (copy_to_user(buf,
			 report_uid ? (void *)reads_buf2 : (void *)reads_buf,
			 result)) {
		result = -EFAULT;
		goto out;
	}
...
out:
	...
	if (reads_buf2)
		free_page((unsigned long)reads_buf2);
	return result;
}
