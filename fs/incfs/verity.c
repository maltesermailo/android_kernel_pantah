...
static struct mem_range fsverity_get_file_digest(struct inode *inode)
{
	...
	for (lvl = 0; lvl <= hash_tree->depth; lvl++) {
		for (i = 0; i < limit; ++i) {
			...
			if (lvl == 0)
				...
				result = incfs_kread(bfc, buf.data, buf.len,
						hash_level + i *
...
/* If file doesn't have a signature or hash tree, add them */
static int add_signature_record(struct file *f)
{
...
/**
 * fsverity_file_open() - prepare to open a file that may be verity-enabled
