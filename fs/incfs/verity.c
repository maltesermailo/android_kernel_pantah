 ...
 *    fs.verity.require_signatures=0, then the S_VERITY flag is set, the digest
 *    is added to the data_file struct and the signature is added to the backing
 *    file. Note that an empty signature can still be added simply by adding
 *    an INCFS_MD_VERITY_SIGNATURE section but with vs_size and vs_offset set to
 *    zero
 * 5. When a file with an INCFS_MD_VERITY_SIGNATURE section is opened, the
 *    inode’s S_VERITY flag is set. If the file doesn’t have a digest, the
 *    file’s digest is calculated as above, checked, and set.
...
static int incfs_end_enable_verity(struct file *filp, u8 *sig, size_t sig_size)
{
	...
	loff_t offset;
	__le64 verity_attr_value = cpu_to_le64(1);
	...
	error = vfs_setxattr(bfc->bc_file->f_path.dentry,
			     INCFS_XATTR_VERITY_NAME,
			     (char *) &verity_attr_value,
			     sizeof(verity_attr_value),	XATTR_CREATE);
	if (error) {
...
static int incfs_add_signature_record(struct file *f)
{
	...
	error = mutex_lock_interruptible(&df->df_build_merkle_tree);
	if (error)
		return error;
...
out:
	mutex_unlock(&df->df_build_merkle_tree);
	kfree(signature);
	kfree(hash_tree);
	return error;
}
 ...
 * set up data_file->df_verity_file_digest if not already done.
 *
