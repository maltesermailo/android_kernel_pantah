...
int fuse_open_backing(struct fuse_args *fa, struct inode *inode,
...
void *fuse_open_finalize(struct fuse_args *fa, struct inode *inode,
			 struct file *file, bool isdir)
{
...
int fuse_create_open_initialize(struct fuse_args *fa,
				struct fuse_create_open_io *fcoio,
...
int fuse_create_open_backing(struct fuse_args *fa, struct inode *dir,
...
void *fuse_create_open_finalize(struct fuse_args *fa, struct inode *dir,
				struct dentry *entry, struct file *file,
				unsigned int flags, umode_t mode)
{
	struct fuse_file *ff = file->private_data;
...
int fuse_releasedir_initialize(struct fuse_args *fa,
			       struct fuse_release_in *fri, struct inode *inode,
			       struct file *file)
{
	struct fuse_file *fuse_file = file->private_data;
...
int fuse_release_backing(struct fuse_args *fa, struct inode *inode,
			 struct file *file)
{
	return 0;
}

void *fuse_release_finalize(struct fuse_args *fa, struct inode *inode,
			    struct file *file)
{
...
void *fuse_flush_finalize(struct fuse_args *fa, struct file *file,
			  fl_owner_t id)
{
	return NULL;
}
...
int fuse_fsync_backing(struct fuse_args *fa, struct file *file, loff_t start,
...
void *fuse_fsync_finalize(struct fuse_args *fa, struct file *file, loff_t start,
			  loff_t end, int datasync)
{
	return NULL;
}
...
int fuse_getxattr_backing(struct fuse_args *fa, struct dentry *dentry,
...
void *fuse_getxattr_finalize(struct fuse_args *fa, struct dentry *dentry,
			     const char *name, void *value, size_t size)
{
...
{
	ssize_t ret = vfs_listxattr(
		get_fuse_dentry(dentry)->backing_path.dentry, list, size);
...
int fuse_removexattr_backing(struct fuse_args *fa, struct dentry *dentry,
			     const char *name)
{
...
void *fuse_removexattr_finalize(struct fuse_args *fa, struct dentry *dentry,
				const char *name)
{
	return NULL;
}

int fuse_file_read_iter_initialize(struct fuse_args *fa,
				   struct fuse_read_in *fri, struct kiocb *iocb,
				   struct iov_iter *to)
{
...
int fuse_file_read_iter_backing(struct fuse_args *fa, struct kiocb *iocb,
				struct iov_iter *to)
{
	struct file *file = iocb->ki_filp;
	struct fuse_file *ff = file->private_data;
...
void *fuse_file_read_iter_finalize(struct fuse_args *fa, struct kiocb *iocb,
				   struct iov_iter *to)
{
	return ERR_PTR(fa->out_args[0].size);
}

int fuse_file_write_iter_initialize(struct fuse_args *fa,
...
int fuse_file_write_iter_backing(struct fuse_args *fa, struct kiocb *iocb,
...
void *fuse_file_write_iter_finalize(struct fuse_args *fa, struct kiocb *iocb,
				    struct iov_iter *from)
{
	struct fuse_write_out *fwo = fa->out_args[0].value;
...
int fuse_file_fallocate_initialize(struct fuse_args *fa,
				   ...
				   struct file *file, int mode, loff_t offset,
				   loff_t length)
{
	struct fuse_file *ff = file->private_data;
...
int fuse_file_fallocate_backing(struct fuse_args *fa, struct file *file,
...
void *fuse_file_fallocate_finalize(struct fuse_args *fa, struct file *file,
				   int mode, loff_t offset, loff_t length)
{
	return NULL;
}
				    ...
				    struct dentry *entry, unsigned int flags)
{
	...
	struct fuse_entry_bpf *feb =
		container_of(febo, struct fuse_entry_bpf, out);
...
int fuse_mknod_initialize(struct fuse_args *fa, struct fuse_mknod_in *fmi,
...
int fuse_mknod_backing(struct fuse_args *fa, struct inode *dir,
...
void *fuse_mknod_finalize(struct fuse_args *fa, struct inode *dir,
			  struct dentry *entry, umode_t mode, dev_t rdev)
{
	return NULL;
}

int fuse_mkdir_initialize(struct fuse_args *fa, struct fuse_mkdir_in *fmi,
			  struct inode *dir, struct dentry *entry, umode_t mode)
{
...
int fuse_mkdir_backing(struct fuse_args *fa, struct inode *dir,
...
void *fuse_mkdir_finalize(struct fuse_args *fa, struct inode *dir,
			  struct dentry *entry, umode_t mode)
{
	return NULL;
}

int fuse_rmdir_initialize(struct fuse_args *fa, struct fuse_dummy_io *dummy,
			  struct inode *dir, struct dentry *entry)
{
	*fa = (struct fuse_args){
		.nodeid = get_node_id(dir),
...
int fuse_rmdir_backing(struct fuse_args *fa, struct inode *dir,
		       struct dentry *entry)
{
	int err = 0;
	struct path backing_path = {};
...
void *fuse_rmdir_finalize(struct fuse_args *fa, struct inode *dir,
			  struct dentry *entry)
{
	return NULL;
}
...
int fuse_rename2_backing(struct fuse_args *fa, struct inode *olddir,
...
void *fuse_rename2_finalize(struct fuse_args *fa, struct inode *olddir,
			    struct dentry *oldent, struct inode *newdir,
			    struct dentry *newent, unsigned int flags)
{
	return NULL;
...
int fuse_rename_backing(struct fuse_args *fa, struct inode *olddir,
			struct dentry *oldent, struct inode *newdir,
			struct dentry *newent)
{
	/* TODO: deal with changing dirs/ents */
...
void *fuse_rename_finalize(struct fuse_args *fa, struct inode *olddir,
			   struct dentry *oldent, struct inode *newdir,
			   struct dentry *newent)
{
	return NULL;
...
int fuse_unlink_initialize(struct fuse_args *fa, struct fuse_dummy_io *dummy,
			   struct inode *dir, struct dentry *entry)
{
	*fa = (struct fuse_args){
		.nodeid = get_node_id(dir),
...
int fuse_unlink_backing(struct fuse_args *fa, struct inode *dir,
			struct dentry *entry)
{
	int err = 0;
	struct path backing_path = {};
...
void *fuse_unlink_finalize(struct fuse_args *fa, struct inode *dir,
			   struct dentry *entry)
{
	return NULL;
}
			    ...
			    u32 request_mask, unsigned int flags)
{
	fgio->fgi = (struct fuse_getattr_in){
		.getattr_flags = flags, .fh = -1, /* TODO is this OK? */
	};
...
int fuse_getattr_backing(struct fuse_args *fa, const struct dentry *entry,
...
void *fuse_getattr_finalize(struct fuse_args *fa, const struct dentry *entry,
			    struct kstat *stat, u32 request_mask,
			    unsigned int flags)
{
...
int fuse_setattr_backing(struct fuse_args *fa, struct dentry *dentry,
...
void *fuse_setattr_finalize(struct fuse_args *fa, struct dentry *dentry,
			    struct iattr *attr, struct file *file)
{
	return NULL;
}
...
int fuse_statfs_backing(struct fuse_args *fa, struct dentry *dentry,
			struct kstatfs *buf)
{
	int err = 0;
	struct path backing_path;
...
void *fuse_statfs_finalize(struct fuse_args *fa, struct dentry *dentry,
			   struct kstatfs *buf)
{
	struct fuse_statfs_out *fso = fa->out_args[0].value;
...
int fuse_get_link_backing(struct fuse_args *fa, struct inode *inode,
			  struct dentry *dentry, struct delayed_call *callback,
			  const char **out)
{
	struct path backing_path;
...
void *fuse_get_link_finalize(struct fuse_args *fa, struct inode *inode,
...
int fuse_symlink_initialize(struct fuse_args *fa, struct fuse_dummy_io *unused,
			    struct inode *dir, struct dentry *entry,
...
int fuse_symlink_backing(struct fuse_args *fa, struct inode *dir,
			 struct dentry *entry, const char *link, int len)
{
	int err = 0;
	struct inode *backing_inode = get_fuse_inode(dir)->backing_inode;
	struct path backing_path = {};
...
void *fuse_symlink_finalize(struct fuse_args *fa, struct inode *dir,
			    struct dentry *entry, const char *link, int len)
{
	return NULL;
}
...
int fuse_readdir_backing(struct fuse_args *fa, struct file *file,
			 struct dir_context *ctx, bool *force_again,
			 bool *allow_force)
{
	struct fuse_file *ff = file->private_data;
...
void *fuse_readdir_finalize(struct fuse_args *fa, struct file *file,
			    struct dir_context *ctx, bool *force_again,
			    bool *allow_force)
{
	int err = 0;
	...
	err = fuse_parse_dirfile(fa->out_args[1].value, fa->out_args[1].size,
				 file, ctx);
