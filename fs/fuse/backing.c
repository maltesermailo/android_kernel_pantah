...
int fuse_handle_override_creds(struct fuse_entry_bpf *feb, struct inode *parent,
				u64 *override_creds) {
	switch (feb->out.cred_action) {
	case FUSE_ACTION_KEEP: {
		if (!parent) {
			*override_creds = FUSE_USE_USER_CREDENTIALS;
			return 0;
		}
		*override_creds = get_fuse_inode(parent)->override_creds;
		break;
	}
	case FUSE_OVERRIDE_CREDENTIALS: {
		*override_creds = FUSE_OVERRIDE_CREDENTIALS;
		break;
	}
        case FUSE_USE_USER_CREDENTIALS: {
		*override_creds = FUSE_USE_USER_CREDENTIALS;
		break;
        }
