...
{
	const union fscrypt_policy *policy = &ci->ci_policy;
	unsigned int dun_bytes = 8;

	if (fscrypt_policy_flags(policy) & FSCRYPT_POLICY_FLAG_DIRECT_KEY)
				   ...
				   bool is_hw_wrapped_key)
{
	...
	struct request_queue **devs;
	static const u8 zeroes[FSCRYPT_MAX_KEY_SIZE] = {0};
	/*
	 * Note: This isn't really a key.  blk_ksm_crypto_key_supported() only
	 * looks at the encryption settings, not the key itself.
	 */
	struct blk_crypto_key enc_settings;
	int num_devs;
	int err;
	int i;
	...
	/*
	 * Either all the filesystem's devices must support the needed crypto
	 * setting in hardware, or blk-crypto-fallback must support it.
	 */
	...
	err = blk_crypto_init_key(&enc_settings, zeroes, ci->ci_mode->keysize,
				  is_hw_wrapped_key, ci->ci_mode->blk_crypto_mode,
	...
	for (i = 0; i < num_devs; i++) {
		if (!blk_ksm_crypto_key_supported(devs[i]->ksm, &enc_settings))
			goto out_free_devs;
	}
