			      ...
			      u8 hkdf_context, bool include_fs_uuid)
{
	...
	if (S_ISREG(inode->i_mode) && mk->mk_secret.is_wrapped_key &&
		(ci->ci_policy.v2.flags & FSCRYPT_POLICY_FLAG_WRAPPED_KEY)) {
		int i;
		if (!fscrypt_using_inline_encryption(ci)) {
			fscrypt_warn(ci->ci_inode,
				     "wrapped keys require inline encryption (-o inlinecrypt)");
			err = -EINVAL;
			goto out_unlock;
		}
		for (i = 0; i <= __FSCRYPT_MODE_MAX; i++) {
			if (fscrypt_is_key_prepared(&keys[i], ci)) {
				fscrypt_warn(ci->ci_inode,
					     "wrapped keys only support one encryption mode at a time");
				err = -EINVAL;
				goto out_unlock;
			}
		}
		...
		if (err)
			goto out_unlock;
		err = fscrypt_prepare_key(prep_key, mode_key, mode->keysize, ci);
				     ...
				     struct fscrypt_master_key *mk)
{
	...
	if (ci->ci_policy.v2.flags & FSCRYPT_POLICY_FLAG_DIRECT_KEY) {
		...
		if (ci->ci_policy.v2.flags & FSCRYPT_POLICY_FLAG_WRAPPED_KEY) {
			if (!mk->mk_secret.is_wrapped_key) {
				fscrypt_warn(ci->ci_inode,
					     "Wrapped key not set with FLAG_WRAPPED_KEY");
				return -EINVAL;
			}
		}
		/*
