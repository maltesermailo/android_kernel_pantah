...
static inline struct sdcardfs_inode_data *grab_data(
		struct sdcardfs_inode_data *data)
{
	if (data)
...
static inline struct sdcardfs_inode_data *grab_top(
		struct sdcardfs_inode_info *info)
{
	return grab_data(info->top_data);
}

extern void info_data_release(struct kref *ref);

static inline void release_data(struct sdcardfs_inode_data *data)
{
	kref_put(&data->refcount, info_data_release);
}

static inline void release_own_data(struct sdcardfs_inode_info *info)
{
	info->data->abandoned = true;
	release_data(info->data);
}
