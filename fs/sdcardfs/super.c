...
void info_data_release(struct kref *ref)
{
...
static struct inode *sdcardfs_alloc_inode(struct super_block *sb)
{
	...
	d = kmem_cache_alloc(sdcardfs_inode_data_cachep, GFP_KERNEL);
	if (!d) {
		kmem_cache_free(sdcardfs_inode_cachep, i);
		return NULL;
	}

	/* memset everything up to 0 */
	memset(d, 0, sizeof(struct sdcardfs_inode_data));
	i->data = d;
