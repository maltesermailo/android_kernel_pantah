...
static int sdcardfs_parse_param(struct fs_context *fc, struct fs_parameter *param)
{
    ...
    switch (opt) {
		case Opt_fsuid:
			mntopts->fs_low_uid = result.uint_32;
			break;
		case Opt_fsgid:
			mntopts->fs_low_gid = result.uint_32;
			break;
		case Opt_gid:
			mntopts->gid = result.uint_32;
			break;
		case Opt_userid:
			mntopts->fs_user_id = result.uint_32;
			break;
		case Opt_mask:
			mntopts->mask = result.uint_32;
			break;
		case Opt_reserved_mb:
			mntopts->reserved_mb = result.uint_32;
			break;
		case Opt_multiuser:
			mntopts->multiuser = true;
			break;
		case Opt_gid_derivation:
			mntopts->gid_derivation = true;
			break;
		case Opt_default_normal:
			mntopts->default_normal = true;
			break;
		case Opt_nocache:
			mntopts->nocache = true;
			break;
		case Opt_unshared_obb:
			mntopts->unshared_obb = true;
			break;
		case Opt_debug:
			mntopts->debug = true;
			break;
		...
		const char *dev_name, struct sdcardfs_mount_options *mntopts, int silent)
{
	...
	options = &sb_info->options;
	options->fs_low_uid = mntopts->fs_low_uid;
	options->fs_low_gid = mntopts->fs_low_gid ;
	options->gid = mntopts->gid;
	options->fs_user_id = mntopts->fs_user_id;
	options->mask = mntopts->mask;
	options->reserved_mb = mntopts->reserved_mb;
	options->gid_derivation = mntopts->gid_derivation;
	options->multiuser = mntopts->multiuser;
	options->default_normal = mntopts->default_normal;
	options->unshared_obb = mntopts->unshared_obb;
	options->nocache = mntopts->nocache;
	options->debug = mntopts->debug;
...
static int sdcardfs_reconfigure(struct fs_context *fc)
{
	...
	struct sdcardfs_mount_options *options = &sb_info->options;
...
static int sdcardfs_init_fs_context(struct fs_context *fc)
{
	...
	mntopts->fs_low_uid = AID_MEDIA_RW;
	mntopts->fs_low_gid = AID_MEDIA_RW;
	mntopts->fs_user_id = 0;
	mntopts->gid = 0;
	mntopts->mask = 0;

	/* by default, 0MB is reserved */
	mntopts->reserved_mb = 0;
	/* by default, gid derivation is off */
	mntopts->gid_derivation = false;
	mntopts->default_normal = false;
	mntopts->nocache = false;
	mntopts->multiuser = false;
	mntopts->debug = false;
