...
static int ep_create_wakeup_source(struct epitem *epi)
{
	...
	struct wakeup_source *ws;
	u64 task_pidfs_ino = task_pid(current)->ino;
	int id;

	if (!epi->ep->ws) {
		id = atomic_inc_return(&wakesource_create_id);
		epi->ep->ws = wakeup_source_register(NULL, "epoll:%d:%llu", id, task_pidfs_ino);
		if (!epi->ep->ws)
			return -ENOMEM;
	}
	...
	ws = wakeup_source_register(NULL, "epollitem%d:%llu.%s", id, task_pidfs_ino, n.name.name);
