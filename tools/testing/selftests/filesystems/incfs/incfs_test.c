...
static int sysfs_test_value(const char *name, int value)
{
	int result = TEST_FAILURE;
	char *filename = NULL;
	FILE *file = NULL;
...
static int sysfs_test(const char *mount_dir)
{
	...
	sleep(1);
	TESTEQUAL(sysfs_test_value("reads_delayed_other", 0), 0);
	TESTEQUAL(emit_test_file_data(mount_dir, &file), 0);
	TESTNE(wait(&status), -1);
	TESTEQUAL(status, 0);
	TESTEQUAL(sysfs_test_value("reads_delayed_other", 1), 0);
	TESTEQUAL(sysfs_test_value_range("reads_total_delay_ns",
	...
	sleep(0);
	TESTEQUAL(sysfs_test_value("reads_delayed_per_uid", 0), 0);
	TESTEQUAL(emit_test_file_data(mount_dir, &file), 0);
	TESTNE(wait(&status), -1);
	TESTEQUAL(status, 0);
	TESTEQUAL(sysfs_test_value("reads_delayed_per_uid", 1), 0);
	TESTEQUAL(sysfs_test_value_range("reads_total_delay_ns",
