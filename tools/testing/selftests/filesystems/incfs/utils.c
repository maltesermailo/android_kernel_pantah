...
int mount_fs(const char *mount_dir, const char *backing_dir, int read_timeout_ms)
{
...
int mount_fs_opt(const char *mount_dir, const char *backing_dir, const char *opt)
{
	static const char fs_name[] = INCFS_NAME;
	int result;
...
void format_signature(void **buf, const char* root_hash, const char* add_data)
{
	struct signature_blob *sb = malloc(sizeof(struct signature_blob)
					   + strlen(add_data));
		     ...
		     const char *add_data)
{
	...
	format_signature(&signature, root_hash, add_data);
...
int emit_file(int fd, const char *dir, const char *filename, incfs_uuid_t *id_out,
		size_t size, const char *attr)
{
	int mode = __S_IFREG | 0555;
	struct incfs_new_file_args args = {
			.size = size,
