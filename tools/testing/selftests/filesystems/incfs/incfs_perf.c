...
int parse_options(int argc, char *const *argv, struct options *options)
{
	...
	*options = (struct options){
	...
	while ((c = getopt(argc, argv, "b:cd:hnors:t:v")) != -1) {
		switch (c) {
		case 'b':
...
{
	int err = measure_read_throughput_internal(tag, dir, name, options,
						   false);
...
int test_incfs_file(int dst_dir, int iname, const struct options *options,
		    int flags)
{
	...
	snprintf(name, sizeof(name), "%03d", iname);
...
int main(int argc, char *const *argv)
{
	...
	for (flags = 0; flags < LAST_FLAG && !err; ++flags) {
		if ((flags & SHUFFLE) && options.no_shuffle)
			continue;
		if ((flags & COMPRESS) && options.no_compress)
			continue;
		if ((flags & VERIFY) && options.no_verify)
			continue;
		err = test_incfs_file(dst_dir_fd, flags, &options, flags);
	}
