...
__section("dummy") __used inline int strcmp(const char *a, const char *b)
{
	int i;
...
__section("maps") __used struct fuse_bpf_map test_map = {
	BPF_MAP_TYPE_ARRAY,
	sizeof(uint32_t),
	sizeof(uint32_t),
	1000,
...
__section("maps") __used struct fuse_bpf_map test_map2 = {
	BPF_MAP_TYPE_HASH,
	sizeof(uint32_t),
	sizeof(uint64_t),
	76,
...
__section("test_daemon") __used int trace_daemon(struct fuse_bpf_args *fa)
{
