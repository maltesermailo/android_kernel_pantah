...
static int bpf_test_readdir(const char *mount_dir)
{
	...
	FUSE_DAEMON
		...
		struct fuse_read_out *read_out =
			(struct fuse_read_out *) (bytes_in + sizeof(*in_header) +
		...
		*fuse_dirent = (struct fuse_dirent) {
		...
		res += FUSE_DIRENT_ALIGN(sizeof(*fuse_dirent) + strlen("fake") +
					 1);
		TESTFUSEDIROUTREAD(read_out, bytes_in + sizeof(struct fuse_in_header)
					+ sizeof(struct fuse_read_in) + sizeof(struct fuse_read_out),
	...
	bool skip = true;
		for (int i = 0; i < ARRAY_SIZE(names) + 1; i++) {
			...
			if (dirs_in < bytes_in + res) {
				...
				is_dot = (!strcmp(fuse_dirent_in->name, ".") || !strcmp(fuse_dirent_in->name, ".."));

				dir_ent_len = FUSE_DIRENT_ALIGN(sizeof(*fuse_dirent_in) + fuse_dirent_in->namelen + 1);
				if (dirs_in + dir_ent_len < bytes_in + res)
					next = (struct fuse_dirent *)(dirs_in + dir_ent_len);

				if (!skip || is_dot) {
					memcpy(dirs_out, fuse_dirent_in, sizeof(struct fuse_dirent) + fuse_dirent_in->namelen + 1);
			...
			fuse_read_out_out->offset = next? next->off : fuse_read_out_in->offset;
			...
			*out_header = (struct fuse_out_header) {
				.len = sizeof(*out_header) + sizeof(*fuse_read_out_out) + length_out,
...
{
	const char file_name[] = "real";
	const char xattr_name[] = "user.xattr_test_name";
	const char xattr_value[] = "this_is_a_test";
