...
static void pkvm_mmio_write(uint64_t addr, uint64_t val)
{
	GUEST_ASSERT(kvm_hypercall(PKVM_GHC_IOWRITE, addr, sizeof(val), val, 0) == 0);
}
		...
		for (j = 0; j < 2; j++) {
			...
			for (i = 0; i < io->ring_size - 1; i++) {
				if (i & 1)
					...
					pkvm_mmio_write(io->mmio_gpa, io->mmio_gpa + i);
			}
			if (j & 1)
				outl(io->pio_port, io->pio_port + i);
			else
