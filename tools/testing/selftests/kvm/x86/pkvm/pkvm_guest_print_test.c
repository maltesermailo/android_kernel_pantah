...
struct shared_guest_vals {
	u64 gpa;
	bool shared;
	struct guest_vals vals;
};

static struct shared_guest_vals *shared_vals_ptr;
static struct shared_guest_vals *shared_vals_hva;
...
{								     \
	...
	shared_vals_hva->vals = (struct guest_vals){			     \
		(uint64_t)a, (uint64_t)b, TYPE_##ext			     \
	};							     \
...
static void guest_share_vals(void)
{
	if (shared_vals_ptr->shared)
		return;

	pkvm_guest_share_mem(shared_vals_ptr, shared_vals_ptr->gpa,
			    sizeof(*shared_vals_ptr));
	shared_vals_ptr->shared = true;
}
...
static void guest_code(void)
{
	...
	while (1) {
		switch (shared_vals_ptr->vals.type) {
#define TYPE(fn, ext, fmt_t, T)						\
		case TYPE_##ext:						\
			GUEST_PRINTF(PRINTF_FMT_##ext,					\
				    shared_vals_ptr->vals.a,				\
				    shared_vals_ptr->vals.b);				\
			__GUEST_ASSERT(shared_vals_ptr->vals.a == shared_vals_ptr->vals.b,\
				       ASSERT_FMT_##ext,				\
				       shared_vals_ptr->vals.a,				\
				       shared_vals_ptr->vals.b);				\
			break;
...
#undef TYPE
		default:
			GUEST_SYNC(shared_vals_ptr->vals.type);
		}
 ...
 * guest asserts can be verified instead of being reported as failures.
 */
...
int main(int argc, char *argv[])
{
	...
	vals_gva = vm_vaddr_alloc(vm, sizeof(*shared_vals_hva), KVM_UTIL_MIN_VADDR);
	shared_vals_hva = addr_gva2hva(vm, vals_gva);
	memset(shared_vals_hva, 0, sizeof(*shared_vals_hva));
	shared_vals_hva->gpa = addr_hva2gpa(vm, shared_vals_hva);
	write_guest_global(vm, shared_vals_ptr, (struct shared_guest_vals *)vals_gva);
