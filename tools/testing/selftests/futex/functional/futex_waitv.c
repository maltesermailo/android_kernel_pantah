...
{
	int res = futex_waitv(NULL, 0, 0, NULL, CLOCK_MONOTONIC);
...
void *waiterfn(void *arg)
{
	...
	if (res < 0) {
		...
		EXPECT_EQ(res, NR_FUTEXES - 1) TH_LOG("futex_waitv returned %d, expected %d", res, NR_FUTEXES - 1);
	}
...
TEST(private_waitv)
{
	...
	/* Private waitv */
	ASSERT_EQ(pthread_create(&waiter, NULL, waiterfn, _metadata), 0) TH_LOG("pthread_create failed");

	usleep(WAKE_WAIT_US);

	res = futex_wake(u64_to_ptr(waitv[NR_FUTEXES - 1].uaddr), 1, FUTEX_PRIVATE_FLAG);
	EXPECT_EQ(res, 1) TH_LOG("futex_wake private returned: %d %s", res, res < 0 ? strerror(errno) : "");
}
...
TEST(shared_waitv)
{
	...
	EXPECT_EQ(res, 1) TH_LOG("futex_wake shared returned: %d %s", res, res < 0 ? strerror(errno) : "");
...
TEST(invalid_flag)
{
	...
	if (res == -1) {
		EXPECT_EQ(errno, EINVAL) TH_LOG("futex_waitv returned unexpected errno: %d", errno);
	}
}
