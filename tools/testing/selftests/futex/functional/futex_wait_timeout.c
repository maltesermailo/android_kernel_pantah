...
#define TEST_TIMEOUT(_res, _test_name, _err) do { \
	...
	EXPECT_EQ((_res), -1) TH_LOG("%s returned unexpected result: %d", _test_name, (_res)); \
	if ((_res) == -1) { \
		EXPECT_EQ(errno, (_err)) TH_LOG("%s returned unexpected errno: %d (expected %d)", _test_name, errno, (_err)); \
	} \
} while (0)
...
TEST(lock_pi)
{
	...
	ASSERT_EQ(pthread_create(&thread, NULL, get_pi_lock, _metadata), 0) TH_LOG("pthread_create failed");
