...
static int contains_operator(char *str, char **sep)
{
	...
	char *minus_op = NULL, *plus_op = NULL, *div_op = NULL, *mult_op = NULL;


	/*
	 * Report the last occurance of the operators first, so that the
				      ...
				      char *var_name, unsigned int *level)
{
	...
	/* Unary minus operator, increment level */
	++*level;
				     ...
				     char *var_name, unsigned int *level)
{
	...
	/* Binary operator found, increment level */
	++*level;
			      ...
			      unsigned long flags)
{
	...
	hist_field = parse_expr(hist_data, file, field_str, flags, var_name, &level);
	if (IS_ERR(hist_field)) {
