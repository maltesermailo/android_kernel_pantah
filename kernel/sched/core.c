				  ...
				  const struct sched_attr *attr)
{
	...
	if (likely(!(attr->sched_flags & SCHED_FLAG_UTIL_HINT)))
		return;

	p->se.avg.util_est.enqueued = attr->sched_util & ~UTIL_AVG_UNCHANGED;
}
...
recheck:
	...
	if (attr->sched_flags & ~(SCHED_FLAG_ALL | SCHED_FLAG_SUGOV | SCHED_FLAG_UTIL_HINT))
		return -EINVAL;
	...
	if (user) {
		...
		if (attr->sched_flags & SCHED_FLAG_UTIL_HINT)
			return -EPERM;
