...
static void __enqueue_entity(struct cfs_rq *cfs_rq, struct sched_entity *se)
{
	...
	trace_android_rvh_RB_enqueue_entity(cfs_rq, se);
	/*
...
{
	trace_android_rvh_RB_dequeue_entity(cfs_rq, se);
...
again:
	...
	p = task_of(se);
	trace_android_rvh_replace_next_task_fair(rq, &p, &se, &repick, false, prev, &find);
	/*
...
simple:
	...
	trace_android_rvh_replace_next_task_fair(rq, &p, &se, &repick, true, prev, &find);
	if (repick)
		goto done;
	if (find) {
		cfs_rq = cfs_rq_of(se);
