			    ...
			    unsigned long weight)
{
	...
	if (se->on_rq) {
		...
		update_curr(cfs_rq);
<<<<<<< HEAD   (7cb75cc32a4c684ab33cd52a42210be6d640c6ff ANDROID: ABI: pixel: update symbol list)
		update_entity_lag(cfs_rq, se);
		se->deadline -= se->vruntime;
		se->rel_deadline = 1;
		cfs_rq->nr_running--;
||||||| BASE   (34962374191abcc7ef32a03f111e6123472011f1 Revert "BACKPORT: sched/fair: Adhere to place_entity() const)
		update_entity_lag(cfs_rq, se);
		se->deadline -= se->vruntime;
		se->rel_deadline = 1;
=======
		avruntime = avg_vruntime(cfs_rq);
>>>>>>> CHANGE (286c463a1eccbb3e14e2fb508b19dcffa1ef92f8 Revert "BACKPORT: sched/fair: Fix EEVDF entity placement bug)
		if (!curr)
