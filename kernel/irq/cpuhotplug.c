...
static bool migrate_one_irq(struct irq_desc *desc)
{
	...
	cpumask_andnot(&available_cpus, &available_cpus, cpu_paused_mask);

	if (cpumask_any_and(&available_cpus, cpu_online_mask) >= nr_cpu_ids) {
		 ...
		 *
		 * (1) online and un-paused CPU from default affinity
		 * (2) online and un-paused CPU
		 * (3) online CPU
		 */
		...
		affinity = cpumask_of(cpumask_any(&available_cpus));
		brokeaff = true;
	}
	else
	{
