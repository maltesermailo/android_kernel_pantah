 ...
 *				contiguous memory.
 * @host_stage2_unmap:		Unmap a range of memory from the host stage-2.
 * 				This range is still owned by the host and can
 * 				be shared. In case of host access, a fault would
 * 				occur and a mapping would be installed. However
 * 				coalescing can't happen until @host_stage2_remap
 * 				is called.
 * @host_stage2_remap:		This is the opposite function of
 * 				host_stage2_unmap. Must be called once the
 * 				module is done with the region. It re-enable
 * 				coalescing remaps the region.
...
struct pkvm_module_ops {
	...
	ANDROID_KABI_USE(3, int (*host_stage2_unmap)(u64 addr, u64 nr_pages));
	ANDROID_KABI_USE(2, int (*host_stage2_remap)(u64 addr, u64 nr_pages));
