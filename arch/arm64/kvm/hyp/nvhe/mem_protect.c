...
enum host_check_page_state_flags {
	HOST_CHECK_NULL_REFCNT	= 1,
	HOST_CHECK_RESTRICT_MMIO,
};
					  ...
					  enum host_check_page_state_flags flags)
{
	...
	if (!reg && (flags & HOST_CHECK_RESTRICT_MMIO))
		return -EINVAL;
...
static int __host_check_page_state_range(u64 addr, u64 size,
					 enum pkvm_page_state state)
{
	enum host_check_page_state_flags flags = HOST_CHECK_RESTRICT_MMIO;
	if (state == PKVM_PAGE_OWNED)
				 ...
				 bool update_iommu)
{
	...
	if (page->host_state & PKVM_MODULE_OWNED_PAGE) {
		...
		ret = ___host_check_page_state_range(
				addr, nr_pages << PAGE_SHIFT, PKVM_PAGE_OWNED, HOST_CHECK_NULL_REFCNT);
		if (ret)
			goto unlock;
	}
...
int __pkvm_host_reclaim_page(struct pkvm_hyp_vm *vm, u64 pfn, u64 ipa, u8 order)
{
	...
	case PKVM_PAGE_OWNED:
		WARN_ON(___host_check_page_state_range(phys, page_size, PKVM_NOPAGE, 0));
