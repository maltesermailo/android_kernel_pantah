				...
				enum kvm_pgtable_prot perms)
{
	...
	return kvm_pgtable_stage2_map(&vm->pgt, checked_tx->completer_addr, size,
				...
				enum kvm_pgtable_walk_flags visit)
{
	...
	if (!data->size) {
		...
		data->ipa_start = ctx->addr &
			(~GENMASK(kvm_granule_shift(level) - 1, 0));
		goto end;
	}
				...
				u64 nr_pages, u64 *guarded)
{
	 ...
	 */
	data.desired_mask = ~KVM_INVALID_PTE_MMIO_NOTE;
			       ...
			       u64 nr_pages, u64 *unguarded)
{
	...
	if (unguarded)
		*unguarded = data.size >> PAGE_SHIFT;
unlock:
	return WARN_ON(ret);
}
