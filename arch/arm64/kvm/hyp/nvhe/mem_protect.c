...
static inline int __host_stage2_idmap(u64 start, u64 end, enum kvm_pgtable_prot prot, bool is_memory)
{
	return kvm_pgtable_stage2_map(&host_mmu.pgt, start, end - start, start, prot,
				      region_to_pool(is_memory), 0);
}
...
{
	return host_stage2_try(__host_stage2_idmap, addr, addr + size, prot, is_memory);
}
