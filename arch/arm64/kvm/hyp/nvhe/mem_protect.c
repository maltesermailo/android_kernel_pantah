...
static int ___pkvm_guest_relinquish_to_module(struct pkvm_hyp_vcpu *vcpu, u64 ipa, u64 *__phys,
					      kvm_pte_t pte, s8 level)
{
				    ...
				    u64 ipa, u64 *ppa)
{
	...
	if (state != PKVM_PAGE_OWNED) {
		ret = ___pkvm_guest_relinquish_to_module(vcpu, ipa, &phys, pte, level);
		goto end;
	}
