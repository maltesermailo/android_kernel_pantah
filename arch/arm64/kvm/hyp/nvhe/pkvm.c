...
static bool pkvm_memshare_call(struct pkvm_hyp_vcpu *hyp_vcpu, u64 *exit_code)
{
	...
	err = __pkvm_guest_share_host(hyp_vcpu, ipa, nr_pages, &shared);
	switch (err) {
	case 0:
...
static bool pkvm_memunshare_call(struct pkvm_hyp_vcpu *hyp_vcpu)
{
	...
	err = __pkvm_guest_unshare_host(hyp_vcpu, ipa, nr_pages, &unshared);
	if (err)
		goto out_guest_err;
				      ...
				      u64 *exit_code)
{
	...
	ret = __pkvm_install_ioguard_page(hyp_vcpu, ipa, nr_pages, &guarded);
				     ...
				     u64 *exit_code)
{
	...
	ret = __pkvm_remove_ioguard_page(hyp_vcpu, ipa, nr_pages, &unguarded);
