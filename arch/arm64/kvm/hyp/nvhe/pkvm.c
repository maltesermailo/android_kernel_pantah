...
static int vm_copy_id_regs(struct pkvm_hyp_vcpu *hyp_vcpu, const struct kvm_vcpu *host_vcpu)
{
	const struct kvm *host_kvm = kern_hyp_va(host_vcpu->kvm);
	struct kvm *kvm = hyp_vcpu->vcpu.kvm;
...
static int pkvm_vcpu_init_sysregs(struct pkvm_hyp_vcpu *hyp_vcpu, const struct kvm_vcpu *host_vcpu)
{
	int ret = 0;
	...
	if (pkvm_hyp_vcpu_is_protected(hyp_vcpu)) {
		...
		ret = vm_copy_id_regs(hyp_vcpu, host_vcpu);
	}
			      ...
			      struct kvm_vcpu *host_vcpu)
{
	...
	ret = pkvm_vcpu_init_sysregs(hyp_vcpu, host_vcpu);
	if (ret)
		goto done;
