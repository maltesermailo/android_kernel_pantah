...
{
	void *sve_state = READ_ONCE(host_vcpu->arch.sve_state);
	...
	if (pkvm_hyp_vcpu_is_protected(hyp_vcpu)) {
		...
		if (!hyp_pin_shared_mem(sve_state, sve_state + sve_state_size))
			goto err;
	}

	hyp_vcpu->vcpu.arch.sve_state = kern_hyp_va(sve_state);
