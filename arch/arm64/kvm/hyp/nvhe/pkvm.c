...
static void unpin_host_vcpus(struct kvm_shadow_vm *vm, int nr_vcpus)
{
	int i;

	for (i = 0; i < nr_vcpus; i++) {
		struct kvm_vcpu *host_vcpu = vm->shadow_vcpus[i].vcpu.arch.pkvm.host_vcpu;

		hyp_unpin_shared_mem(host_vcpu, host_vcpu + 1);
	}
}

static int set_host_vcpus(struct kvm_shadow_vm *vm, int nr_vcpus, void *pgd, size_t pgd_size)
{
	struct kvm_vcpu **vcpu_array;
	int i;

	if (pgd_size < sizeof(struct kvm_vcpu *) * nr_vcpus)
		return -EINVAL;
	...
	for (i = 0; i < nr_vcpus; i++) {
		...
		if (hyp_pin_shared_mem(host_vcpu, host_vcpu + 1)) {
			unpin_host_vcpus(vm, i);
			return -EBUSY;
		}

		vm->shadow_vcpus[i].vcpu.arch.pkvm.host_vcpu = host_vcpu;
	}
		       ...
		       void *pgd)
{
	...
	ret = set_host_vcpus(vm, nr_vcpus, pgd, pgd_size);
	if (ret)
		goto err_remove_pgd;
...
int __pkvm_teardown_shadow(int shadow_handle)
{
	...
	unpin_host_vcpus(vm, vm->created_vcpus);
