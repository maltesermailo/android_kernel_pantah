...
bool kvm_handle_pvm_smc64(struct kvm_vcpu *vcpu, u64 *exit_code)
{
	...
	if (smp_load_acquire(&vm->smc_handler)) {
		...
		handled = vm->smc_handler(&regs, vm->kvm.arch.pkvm.handle);

		/* Pass the return back to the calling guest */
		ctxt->regs.regs[0] = regs.a0;
	}
...
int __pkvm_register_guest_smc_handler(bool (*cb)(struct arm_smccc_1_2_regs *, pkvm_handle_t handle),
