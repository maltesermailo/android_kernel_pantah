...
static inline int pg_pte_from_addr_smpt(u32 *smpt, u64 addr)
{
...
{
	int pte = pg_pte_from_addr_smpt(smpt, addr);
...
static inline bool __is_smpt_uniform(u32 *smpt, enum mpt_prot prot)
{
	...
	//we need to mask granulity bits when doing comparison for access only
	u64 prot_mask =	config_lut_prot[MPT_PROT_MASK];

	for (i = 0; i < SMPT_NUM_WORDS(config_prot_bits) / 2; i++) {
		if ((doublewords[i] & prot_mask) != config_lut_prot[prot])
			return false;
	}
	return true;
}
...
static void pg_write_l1entry_attr_l2(bool l2_enable, u32 gran, enum mpt_prot prot,
...
static void pg_init_with_mpt(void *dev_va, struct mpt *mpt)
{
	...
	for_each_gb_and_vid(gb, vid) {
		...
		pg_write_l1entry_attr_l2(!fmpt->gran_1g, SMPT_GRAN_ATTR, fmpt->prot,
				vid, gb, dev_va);
	}
}
...
static void	 pg_prepare_range(phys_addr_t  first_byte, phys_addr_t last_byte, struct mpt *mpt,
							enum mpt_prot prot)
{
...
static const struct s2mpu_pgtable_ops this_ops = {
	...
	.pte_from_addr_smpt = pg_pte_from_addr_smpt,
};
