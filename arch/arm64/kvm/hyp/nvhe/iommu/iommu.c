...
static int kvm_iommu_init_cma_pool(void)
{
	...
	if (!is_power_of_2(cma_size))
		return -EINVAL;

	/*
	 * Donate the pages temporarily, at this point the CMA is not created
	 * yet in EL1 so the memory is reserved.
	 */
	ret = __pkvm_host_donate_hyp(cma_pfn, cma_nr_pages);
	if (ret)
		return ret;

	ret = hyp_pool_init(&iommu_cma_pool, cma_pfn, cma_nr_pages, 0);
	if (ret)
		goto out_donate_host;

	__hyp_pool_set_range_reclaimable(&iommu_cma_pool);

	/*
	 * Allocate and immediately reclaim and donate the pages back
	 * to the host so it can init the CMA pool.
	 */
	for (i = 0 ; i < cma_nr_pages ; ++i) {
		void *p;

		p = hyp_alloc_pages(&iommu_cma_pool, 0);
		WARN_ON(!p);

		WARN_ON(hyp_pool_reclaim(&iommu_cma_pool, hyp_virt_to_page(p), 0, true));
	}

out_donate_host:
	WARN_ON(__pkvm_hyp_donate_host(cma_pfn, cma_nr_pages));
	return ret;
}
				      ...
				      u8 order, int flags)
{
	...
	if (cma_size && !hyp_vcpu) {
		p = hyp_alloc_pages(&iommu_cma_pool, order);
		if (p)
			return p;
	}
