			 ...
			 u32 endpoint_id, u32 pasid, u32 pasid_bits)
{
	...
	ret = kvm_iommu_ops->attach_dev(iommu, domain_id, domain,
					endpoint_id, pasid, pasid_bits);
	if (ret)
		goto err_put_domain;
			     ...
			     unsigned long iova, size_t pgsize, size_t pgcount)
{
	...
	while (total_unmapped < size) {
		...
		unmapped = iopt_unmap_pages_walk(&iopt, iova, pgsize, max_pgcount,
						 &iotlb_gather, &walker);
		if (!unmapped)
			goto out_put_domain;
		kvm_iommu_iotlb_sync(iopt.cookie, &iotlb_gather);
