...
static inline int pkvm_to_iommu_prot(enum kvm_pgtable_prot prot, u64 addr)
{
	...
	switch (prot & (KVM_PGTABLE_PROT_X | KVM_PGTABLE_PROT_PXN | KVM_PGTABLE_PROT_UXN)) {
	case KVM_PGTABLE_PROT_PXN:
	case 0:
		iommu_prot |= IOMMU_NOEXEC;
		break;
	/* UXN is irrelevant for DMA operations, ignore it */
	case KVM_PGTABLE_PROT_UXN:
	case KVM_PGTABLE_PROT_X:
		break;
	/* X / UXN / PXN are mutually exclusive */
	default:
		WARN_ON(1);
	}
