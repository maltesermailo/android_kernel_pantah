...
/*
 * __get_vcpu:	Returns current running vcpu, this is only called if
 *				the CPU is running and needed when the vcpu context is
 *				required, as for the per-cpu requests.
 * __get_vm: 	Returns current context, that also include teardown of a VM,
 *				this is needed as the core hypervisor can free guest domains
 *				after a guest dies. However the teardown has to be done from
 *				the VM context so the memory go back to the correct pool.
 */
