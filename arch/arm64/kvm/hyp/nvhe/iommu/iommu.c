...
handle_to_domain(pkvm_handle_t domain_id)
{
	...
	if (!domains) {
		if (domain_id == KVM_IOMMU_DOMAIN_IDMAP_ID)
			domains = kvm_iommu_donate_pages_idmap(0);
		else
		...
		if (WARN_ON(cmpxchg64_relaxed(&kvm_hyp_iommu_domains[idx], 0,
					      (void *)domains) != 0)) {
			if (domain_id == KVM_IOMMU_DOMAIN_IDMAP_ID)
				kvm_iommu_reclaim_pages_idmap(domains, 0);
			else
...
static int kvm_iommu_init_idmap(struct kvm_hyp_memcache *idmap_mc)
{
	int ret;
	...
	if (!idmap_mc->head)
		return 0;
	ret = hyp_pool_init_empty(&iommu_idmap_pool, 64 /* order = 6*/);
	if (ret)
		return ret;

	ret = refill_hyp_pool(&iommu_idmap_pool, idmap_mc);
		   ...
		   unsigned long init_arg)
{
	...
	return kvm_iommu_init_idmap(idmap_mc);
}
...
{
	struct kvm_hyp_iommu_domain *domain;
	int iommu_prot = prot ? IOMMU_READ | IOMMU_WRITE : 0;

	if (!kvm_iommu_is_ready())
		return;

	if (iommu_prot && !addr_is_memory(start))
	...
	kvm_iommu_ops->host_stage2_idmap(domain, start, end, iommu_prot);
}
