...
void *mod_handlers[NUM_MOD_HANDLER_TYPES][MAX_MOD_HANDLERS];
...
static int mod_handler_register(enum mod_handler_type type, void *handler)
{
	...
	for (i = 0; i < MAX_MOD_HANDLERS; i++) {
		if (!mod_handlers[type][i]) {
			smp_store_release(&mod_handlers[type][i], handler);
			break;
		}
	}
	...
	return i >= MAX_MOD_HANDLERS ? -EBUSY : 0;
}

static inline void *__get_mod_handler(enum mod_handler_type type, int i)
{
	WARN_ON(type >= NUM_MOD_HANDLER_TYPES);
...
#define for_each_mod_handler(type, handler)				\
	int __i = 0;							\
	for (handler = (typeof(handler))__get_mod_handler(type, 0);	\
	     __i < MAX_MOD_HANDLERS;					\
	     handler = (typeof(handler))__get_mod_handler(type, ++__i))\
	     if (!handler) {						\
		     break;						\
	     } else
...
int module_handle_host_perm_fault(struct user_pt_regs *regs, u64 esr, u64 addr)
{
	...
	for_each_mod_handler(HOST_FAULT_HANDLER, cb) {
...
bool module_handle_host_smc(struct user_pt_regs *regs)
{
	...
	for_each_mod_handler(HOST_SMC_HANDLER, cb)
		handled |= cb(regs);
