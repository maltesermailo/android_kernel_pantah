			    ...
			    struct pkvm_hyp_vcpu *hyp_vcpu)
{
	...
	ret = kvm_notify_vm_availability(hyp_vcpu_to_ffa_handle(hyp_vcpu), FFA_VM_CREATION_MSG);
	if (ret != FFA_RET_SUCCESS)
		goto out;
...
{
	struct ffa_translation *tr = hyp_alloc(sizeof(struct ffa_translation));
	if (!tr)
		return -ENOMEM;
...
static void do_ffa_guest_version(struct arm_smccc_res *res, struct kvm_cpu_context *ctxt, struct pkvm_hyp_vcpu *hyp_vcpu)
{
	DECLARE_REG(u32, ffa_req_version, ctxt, 1);
	...
	if (ret == -EFAULT || ret == -ENOMEM) {
		...
		if (hyp_alloc_ret == -ENOMEM) {
			...
			*exit_code = ARM_EXCEPTION_HYP_REQ;
			...
			ffa_to_smccc_res(&res, linux_errno_to_ffa(hyp_alloc_ret));
			goto out_guest;
		}
...
int kvm_dying_guest_reclaim_ffa_resources(struct pkvm_hyp_vm *vm)
{
	...
	ret = kvm_notify_vm_availability(vm->kvm.arch.pkvm.handle + 1 + HOST_FFA_ID, FFA_VM_DESTRUCTION_MSG);
	if (ret != FFA_RET_SUCCESS)
		goto unlock;
