...
static hyp_spinlock_t kvm_ffa_hyp_lock;
			      ...
			      u16 vm_handle)
{
	...
	int ret = 0;
	struct kvm_ffa_buffers *buffers;
	...
	buffers = &endp_buffers[vm_handle];
	...
	if (vm_handle == HOST_FFA_ID) {
		hyp_unpin_shared_mem(buffers->tx, buffers->tx + 1);
		WARN_ON(__pkvm_host_unshare_hyp(hyp_virt_to_pfn(buffers->tx)));

		hyp_unpin_shared_mem(buffers->rx, buffers->rx + 1);
		WARN_ON(__pkvm_host_unshare_hyp(hyp_virt_to_pfn(buffers->rx)));
	} else {
		pkvm_vcpu = PKVM_VCPU_FROM_CTXT(ctxt);
		WARN_ON(__pkvm_guest_unshare_hyp_page(pkvm_vcpu, buffers->tx_ipa));
		WARN_ON(__pkvm_guest_unshare_hyp_page(pkvm_vcpu, buffers->rx_ipa));
	}
...
int kvm_reclaim_ffa_guest_pages(struct pkvm_hyp_vm *vm, pkvm_handle_t handle)
{
	...
	if (endp_buffers[vm_handle].tx) {
		WARN_ON(__pkvm_guest_unshare_hyp_page(hyp_vcpu, endp_buffers[vm_handle].tx_ipa));
		endp_buffers[vm_handle].tx = NULL;
	}

	if (endp_buffers[vm_handle].rx) {
		WARN_ON(__pkvm_guest_unshare_hyp_page(hyp_vcpu, endp_buffers[vm_handle].rx_ipa));
...
int hyp_ffa_init(void *pages)
{
	...
	kvm_ffa_hyp_lock = __HYP_SPIN_LOCK_UNLOCKED;
