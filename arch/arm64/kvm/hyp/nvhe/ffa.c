			      ...
			      u16 vm_handle)
{
	...
	if (vm_handle == HOST_FFA_ID) {
		...
		WARN_ON(__pkvm_guest_unshare_hyp_page(pkvm_vcpu, endp_buffers[vm_handle].tx_ipa));
		WARN_ON(__pkvm_guest_unshare_hyp_page(pkvm_vcpu, endp_buffers[vm_handle].rx_ipa));
	}
