...
#define FFA_VM_AVAIL_SPS_UNINIT	(-1)
#define FFA_VM_AVAIL_SPS_OOM	(-2)

/*
 * Special value for the VM availability that marks
 * that all the SPs have been notified
 */
#define FFA_VM_AVAIL_NOTIFIED_ALL	((u64)ULLONG_MAX)
...
static s32 num_vm_avail_sps = FFA_VM_AVAIL_SPS_UNINIT;
				      ...
				      u32 availability_msg)
{
	...
	if (availability_msg == FFA_VM_CREATION_MSG)
		smp_store_release(&ffa_buf->vm_avail_bitmap,
				  FFA_VM_AVAIL_NOTIFIED_ALL);
...
bool kvm_guest_ffa_handler(struct pkvm_hyp_vcpu *hyp_vcpu, u64 *exit_code)
{
	...
	ffa_buf = ffa_get_buffers(hyp_vcpu);
	if (READ_ONCE(ffa_buf->vm_avail_bitmap) != FFA_VM_AVAIL_NOTIFIED_ALL) {
		hyp_spin_lock(&kvm_ffa_hyp_lock);
...
out_guest:
	if (ffa_call_needs_smccc_1_2(func_id)) {
		...
		ffa_set_retval(func_id, ctxt, &res);
	}
	return true;
}
...
int kvm_dying_guest_notify(struct pkvm_hyp_vm *vm)
{
	int ret = 0;

	hyp_spin_lock(&kvm_ffa_hyp_lock);
	ret = kvm_notify_vm_availability(hyp_vcpu_to_ffa_handle(vm->vcpus[0]),
					 &vm->ffa_buf, FFA_VM_DESTRUCTION_MSG);
	hyp_spin_unlock(&kvm_ffa_hyp_lock);
