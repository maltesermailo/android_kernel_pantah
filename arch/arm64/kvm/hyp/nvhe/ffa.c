...
static struct ffa_handle *spm_handles;
...
static int ffa_host_store_handle(u64 ffa_handle, bool is_lend)
{
	...
	if (!static_branch_likely(&kvm_ffa_unmap_on_lend))
		return 0;
	...
	spm_handles[i].handle = ffa_handle;
	spm_handles[i].is_lend = is_lend;
	return 0;
}
...
int hyp_ffa_init(void *pages)
{
	...
	if (static_branch_unlikely(&kvm_ffa_unmap_on_lend)) {
		...
		num_spm_handles = KVM_FFA_SPM_HANDLE_NR_PAGES / sizeof(struct ffa_handle);
