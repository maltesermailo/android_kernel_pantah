...
static int ffa_guest_share_with_cb(struct pkvm_hyp_vcpu *vcpu,
				   ...
				   phys_addr_t guest_ipa, u64 *hyp_va, u64 *exit_code)
{
	struct kvm_hyp_req *req;
	int ret = share_cb(vcpu, guest_ipa, hyp_va);
	...
	case -EFAULT:
		...
		req->map.guest_ipa = guest_ipa;
		req->map.size = PAGE_SIZE;
		*exit_code |= REPLAY_FFA_CALL;
		fallthrough;
	case -ENOMEM:
		if (!pkvm_handle_empty_memcache(vcpu, exit_code))
			*exit_code |= REPLAY_FFA_CALL;
	}

	return linux_errno_to_ffa(ret);
}
...
static void *ffa_alloc(size_t size, struct pkvm_hyp_vcpu *vcpu, u64 *exit_code)
{
	...
	if (!buf) {
		...
		if (req) {
			...
			*exit_code = ARM_EXCEPTION_HYP_REQ | REPLAY_FFA_CALL;
		}
...
static int ffa_map_guest_buffers(u64 *tx_va, u64 *rx_va, phys_addr_t tx_ipa,
				 phys_addr_t rx_ipa, struct pkvm_hyp_vcpu *vcpu,
				 u64 *exit_code)
{
	int ret;

	ret = ffa_guest_share_with_cb(vcpu, __pkvm_guest_share_hyp_page, tx_ipa, tx_va, exit_code);
	if (ret)
		return ret;

	ret = ffa_guest_share_with_cb(vcpu, __pkvm_guest_share_hyp_page, rx_ipa, rx_va, exit_code);
	if (ret)
		goto err_unshare_tx;

	return FFA_RET_SUCCESS;
err_unshare_tx:
	WARN_ON(__pkvm_guest_unshare_hyp_page(vcpu, tx_ipa));
	return ret;
}

static int ffa_map_host_buffers(u64 *tx_va, u64 *rx_va, phys_addr_t tx,
				phys_addr_t rx, u32 npages)
{
	int ret;
	...
	*tx_va = (u64)hyp_phys_to_virt(tx);
	ret = hyp_pin_shared_mem((void *)*tx_va, (void *)*tx_va + 1);
	if (ret) {
		ret = FFA_RET_INVALID_PARAMETERS;
		goto err_unshare_rx;
	}

	*rx_va = (u64)hyp_phys_to_virt(rx);
	ret = hyp_pin_shared_mem((void *)*rx_va, (void *)*rx_va + 1);
	if (ret) {
		ret = FFA_RET_INVALID_PARAMETERS;
		goto err_unpin_tx;
	}
	...
	return 0;
err_unpin_tx:
	hyp_unpin_shared_mem((void *)*tx_va, (void *)*tx_va + 1);
err_unshare_rx:
	__pkvm_host_unshare_hyp(hyp_phys_to_pfn(rx));
err_unshare_tx:
	__pkvm_host_unshare_hyp(hyp_phys_to_pfn(tx));
err_unmap_hyp_buffers:
...
static int kvm_init_registered_sp_ids(void)
{
	...
	count = res.a2 < FFA_MAX_REGISTERED_SP_IDS ? res.a2 : FFA_MAX_REGISTERED_SP_IDS;
	num_registered_sp_ids = 0;
	for (i = 0; i < count; i++) {
			   ...
			   u64 *exit_code)
{
	...
	if (!hyp_vcpu)
		...
		ret = ffa_map_guest_buffers(&tx_va, &rx_va, tx, rx, hyp_vcpu, exit_code);
	if (ret)
		goto out_unlock;
			      ...
			      struct pkvm_hyp_vcpu *hyp_vcpu)
{
	...
	if (FFA_HANDLE_FROM_HYP_VCPU(hyp_vcpu) != id) {
		ret = FFA_RET_INVALID_PARAMETERS;
		goto out;
	}
				    ...
				    struct ffa_mem_transfer *transfer)
{
	...
	int i, ret = FFA_RET_SUCCESS;

	for (i = 0; i < nranges; i++) {
		range = &ranges[i];
		translation = ffa_find_translation(transfer, range->address);
		if (!translation) {
			/* At the very best try to unshare the other pages */
			ret = FFA_RET_NO_DATA;
			continue;
		}
				  ...
				  u64 *exit_code)
{
	...
	for (i = 0; i < nranges; i++) {
		...
		for (j = 0; j < range->pg_cnt; j++) {
			...
			ret = ffa_guest_share_with_cb(vcpu, __pkvm_guest_share_ffa_page, ipa, &pa,
						      exit_code);
			if (ret)
				goto unshare;
			     ...
			     u64 *exit_code)
{
	...
	if (hyp_vcpu) {
		...
		transfer = ffa_alloc(sizeof(struct ffa_mem_transfer), hyp_vcpu, exit_code);
		if (IS_ERR(transfer)) {
			ret = linux_errno_to_ffa(PTR_ERR(transfer));
			goto out;
		}
	...
	if (!offset || buf->ep_count != 1 || buf->sender_id != FFA_HANDLE_FROM_HYP_VCPU(hyp_vcpu)) {
		ret = FFA_RET_INVALID_PARAMETERS;
		goto out_unlock;
	}
...
err_unshare:
	if (hyp_vcpu)
		ret = ffa_guest_unshare_ranges(reg->constituents, nr_ranges, hyp_vcpu, transfer);
	else
		WARN_ON(ffa_host_unshare_ranges(reg->constituents, nr_ranges));
	goto out_unlock;
}

#define do_ffa_mem_xfer(fid, res, ctxt, hyp_vcpu, exit_code)	({\
		BUILD_BUG_ON((fid) != FFA_FN64_MEM_SHARE &&	\
			     (fid) != FFA_FN64_MEM_LEND);	\
		__do_ffa_mem_xfer((fid), (res), (ctxt), (hyp_vcpu), (exit_code));\
	})
			   ...
			   struct pkvm_hyp_vcpu *hyp_vcpu)
{
	...
	if (has_version_negotiated) {
		...
		/* Prevent the guest from negotiating a version with TZ */
		res->a0 = FFA_RET_NOT_SUPPORTED;
		goto unlock;
	}
...
bool kvm_host_ffa_handler(struct kvm_cpu_context *host_ctxt, u32 func_id)
{
	...
	case FFA_FN64_RXTX_MAP:
		do_ffa_rxtx_map(&res, host_ctxt, NULL, NULL);
		goto out_handled;
	case FFA_RXTX_UNMAP:
	...
	case FFA_MEM_SHARE:
	case FFA_FN64_MEM_SHARE:
		do_ffa_mem_xfer(FFA_FN64_MEM_SHARE, &res, host_ctxt, NULL, NULL);
		goto out_handled;
	...
	case FFA_MEM_LEND:
	case FFA_FN64_MEM_LEND:
		do_ffa_mem_xfer(FFA_FN64_MEM_LEND, &res, host_ctxt, NULL, NULL);
		goto out_handled;
	case FFA_MEM_FRAG_TX:
		do_ffa_mem_frag_tx(&res, host_ctxt, NULL);
		goto out_handled;
	case FFA_VERSION:
		do_ffa_version(&res, host_ctxt, NULL);
		goto out_handled;
...
bool kvm_guest_ffa_handler(struct pkvm_hyp_vcpu *hyp_vcpu, u64 *exit_code)
{
	...
	case FFA_FEATURES:
		...
		break;
	case FFA_VERSION:
		do_ffa_version(&res, ctxt, hyp_vcpu);
		break;
	case FFA_FN64_RXTX_MAP:
		ret = do_ffa_rxtx_map(&res, ctxt, hyp_vcpu, exit_code);
		break;
	case FFA_RXTX_UNMAP:
	...
	case FFA_MEM_SHARE:
	case FFA_FN64_MEM_SHARE:
		ret = do_ffa_mem_xfer(FFA_FN64_MEM_SHARE, &res, ctxt, hyp_vcpu, exit_code);
		break;
	case FFA_MEM_RECLAIM:
...
static void kvm_guest_clear_transfer(struct ffa_mem_transfer *transfer, struct pkvm_hyp_vcpu *hyp_vcpu)
{
	...
	list_for_each_entry_safe(translation, tmp, &transfer->translations, node) {
		WARN_ON(__pkvm_guest_unshare_ffa_page(hyp_vcpu, translation->ipa));
		list_del(&translation->node);
		hyp_free(translation);
	}
}
...
int kvm_ffa_notify_vm_creation(struct pkvm_hyp_vm *vm, pkvm_handle_t handle)
{
	...
	if (vm) {
		vm_handle = FFA_HANDLE_FROM_PKVM(handle);
		ffa_buf = &vm->ffa_buf;
	} else {
		vm_handle = HOST_FFA_ID;
		ffa_buf = &host_buffers;
	}
...
int kvm_reclaim_ffa_guest_pages(struct pkvm_hyp_vm *vm, pkvm_handle_t handle)
{
	...
	hyp_spin_lock(&kvm_ffa_hyp_lock);
	guest_has_ffa = ffa_buf->tx || ffa_buf->rx;
	if (!guest_has_ffa)
		goto unlock;
...
int hyp_ffa_init(void *pages)
{
	...
	ret = kvm_init_registered_sp_ids();
	if (ret)
		return ret;
