...
static s32 num_registered_sp_ids;

static struct kvm_ffa_buffers *ffa_get_buffers(u16 vm_handle)
{
				 ...
				 u64 *exit_code)
{
	...
	ret = ffa_guest_share_with_cb(vcpu, __pkvm_guest_share_hyp_page, tx_ipa, hyp_tx_va, exit_code);
	if (ret)
		return ret;

	ret = ffa_guest_share_with_cb(vcpu, __pkvm_guest_share_hyp_page, rx_ipa, hyp_rx_va, exit_code);
	if (ret)
		goto err_unshare_tx;
				      ...
				      u32 availability_msg)
{
	...
	for (i = 0; i < num_registered_sp_ids; i++) {
		u64 sp_mask = 1U << i;
		u64 avail_mask = avail_bit << i;
		uint32_t dest = ((uint32_t)sp_ids[i] << 16) | hyp_smp_processor_id();

		if ((ffa_buf->vm_avail_bitmap & sp_mask) == avail_mask)
			continue;
		...
		ffa_buf->vm_avail_bitmap |= avail_mask;
	}

	return FFA_RET_SUCCESS;
}

static int do_ffa_rxtx_map(struct arm_smccc_res *res, struct kvm_cpu_context *ctxt, u16 vm_handle,
			   u64 *exit_code)
{
			      ...
			      u16 vm_handle)
{
	...
	if (vm_handle == HOST_FFA_ID) {
		...
		WARN_ON(__pkvm_guest_unshare_hyp_page(pkvm_vcpu, ffa_buf->tx_ipa));
		WARN_ON(__pkvm_guest_unshare_hyp_page(pkvm_vcpu, ffa_buf->rx_ipa));
	}
				  ...
				  u64 *exit_code)
{
	...
	for (i = 0; i < nranges; i++) {
		...
		for (j = 0; j < range->pg_cnt; j++) {
			...
			ret = ffa_guest_share_with_cb(vcpu, __pkvm_guest_share_ffa_page, ipa, (void **)&pa,
						      exit_code);
			if (ret)
				goto unshare;
...
static int __do_ffa_mem_xfer(const u64 func_id, struct arm_smccc_res *res, struct kvm_cpu_context *ctxt,
			     u16 vm_handle, u64 *exit_code)
{
...
#define do_ffa_mem_xfer(fid, res, ctxt, handle, exit_code)	({\
	int __ret = 0;					\
	do {							\
		BUILD_BUG_ON((fid) != FFA_FN64_MEM_SHARE &&	\
			     (fid) != FFA_FN64_MEM_LEND);	\
		__ret = __do_ffa_mem_xfer((fid), (res), (ctxt), (handle), (exit_code));\
	} while (0); \
	__ret;\
	})
...
bool kvm_host_ffa_handler(struct kvm_cpu_context *host_ctxt, u32 func_id)
{
	...
	case FFA_FN64_RXTX_MAP:
		do_ffa_rxtx_map(&res, host_ctxt, HOST_FFA_ID, NULL);
		goto out_handled;
	case FFA_RXTX_UNMAP:
		do_ffa_rxtx_unmap(&res, host_ctxt, HOST_FFA_ID);
		goto out_handled;
	case FFA_MEM_SHARE:
	case FFA_FN64_MEM_SHARE:
		do_ffa_mem_xfer(FFA_FN64_MEM_SHARE, &res, host_ctxt, HOST_FFA_ID, NULL);
		goto out_handled;
	case FFA_MEM_RECLAIM:
		do_ffa_mem_reclaim(&res, host_ctxt, HOST_FFA_ID);
		goto out_handled;
	case FFA_MEM_LEND:
	case FFA_FN64_MEM_LEND:
		do_ffa_mem_xfer(FFA_FN64_MEM_LEND, &res, host_ctxt, HOST_FFA_ID, NULL);
		goto out_handled;
	case FFA_MEM_FRAG_TX:
		do_ffa_mem_frag_tx(&res, host_ctxt, HOST_FFA_ID);
		goto out_handled;
	case FFA_VERSION:
...
bool kvm_guest_ffa_handler(struct pkvm_hyp_vcpu *hyp_vcpu, u64 *exit_code)
{
	...
	case FFA_MEM_SHARE:
	case FFA_FN64_MEM_SHARE:
		ret = do_ffa_mem_xfer(FFA_FN64_MEM_SHARE, &res, ctxt, vm_handle, exit_code);
		break;
	case FFA_MEM_RECLAIM:
	...
	case FFA_MEM_LEND:
	case FFA_FN64_MEM_LEND:
		ret = do_ffa_mem_xfer(FFA_FN64_MEM_LEND, &res, ctxt, vm_handle, exit_code);
		break;
	case FFA_ID_GET:
