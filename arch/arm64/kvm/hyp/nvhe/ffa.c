			     ...
			     struct pkvm_hyp_vcpu *hyp_vcpu)
{
	...
	if (len > ffa_desc_buf.len) {
		ffa_to_smccc_1_2_error(regs, FFA_RET_NO_MEMORY);
		goto out_unlock;
	}
	...
	if (fraglen != len) {
		...
		ffa_handle = PACK_HANDLE(regs->a1, regs->a2);
	} else if (regs->a0 == FFA_SUCCESS) {
		ffa_handle = PACK_HANDLE(regs->a2, regs->a3);
	} else {
		goto err_unshare;
	}
...
#define do_ffa_mem_xfer(fid, regs, ctxt, hyp_vcpu)	({\
		BUILD_BUG_ON((fid) != FFA_FN64_MEM_SHARE &&	\
			     (fid) != FFA_FN64_MEM_LEND);	\
		__do_ffa_mem_xfer((fid), (regs), (ctxt), (hyp_vcpu));\
	})
				 ...
				 struct pkvm_hyp_vcpu *hyp_vcpu)
{
	...
	/*
	 * For now, FF-A 1.2 is the minimum required guest version because it is
	 * otherwise necessary add non-trivial 1.0 and 1.1 compatibility paths.
	 */
			    ...
			    struct pkvm_hyp_vcpu *hyp_vcpu)
{
	...
	do_ffa_part_get_response(regs, uuid0, uuid1, uuid2, uuid3, flags, ffa_buf);
out_unlock:
	hyp_spin_unlock(&kvm_ffa_hyp_lock);
}

static void do_ffa_direct_msg(struct arm_smccc_1_2_regs *regs,
			      struct kvm_cpu_context *ctxt,
			      u64 vm_handle)
{
...
static int kvm_host_ffa_signal_availability(void)
{
	...
	do_ffa_part_get_response(&regs, 0, 0, 0, 0, 0, NULL);
	if (regs.a0 != FFA_SUCCESS)
		return ffa_to_linux_errno(ret);
...
bool kvm_host_ffa_handler(struct kvm_cpu_context *host_ctxt, u32 func_id)
{
	...
	if (func_id != FFA_VERSION && !has_host_signalled) {
		...
		if (!ret)
			...
			if (host_ctxt->regs.pstate & PSR_I_BIT) {
				ffa_to_smccc_1_2_error(&regs, FFA_RET_DENIED);
				goto out_handled;
			}
	...
	/* Memory management */
	case FFA_FN64_RXTX_MAP:
		do_ffa_rxtx_map(&regs, host_ctxt, NULL);
		goto out_handled;
	case FFA_RXTX_UNMAP:
		do_ffa_rxtx_unmap(&regs, host_ctxt, NULL);
		goto out_handled;
	case FFA_MEM_SHARE:
	case FFA_FN64_MEM_SHARE:
		do_ffa_mem_xfer(FFA_FN64_MEM_SHARE, &regs, host_ctxt, NULL);
		goto out_handled;
	case FFA_MEM_RECLAIM:
		do_ffa_mem_reclaim(&regs, host_ctxt, NULL);
		goto out_handled;
	case FFA_MEM_LEND:
	case FFA_FN64_MEM_LEND:
		do_ffa_mem_xfer(FFA_FN64_MEM_LEND, &regs, host_ctxt, NULL);
		goto out_handled;
	case FFA_MEM_FRAG_TX:
		do_ffa_mem_frag_tx(&regs, host_ctxt, NULL);
		goto out_handled;
	case FFA_VERSION:
		do_ffa_version(&regs, host_ctxt);
		goto out_handled;
	case FFA_PARTITION_INFO_GET:
		do_ffa_part_get(&regs, host_ctxt, NULL);
		goto out_handled;
	case FFA_RX_RELEASE:
		hyp_spin_lock(&kvm_ffa_hyp_lock);
		ffa_rx_release(&regs);
		...
		goto out_handled;
	case FFA_ID_GET:
		ffa_to_smccc_regs_prop(&regs, FFA_RET_SUCCESS, HOST_FFA_ID);
		goto out_handled;
	case FFA_MSG_SEND_DIRECT_REQ:
	case FFA_FN64_MSG_SEND_DIRECT_REQ:
		do_ffa_direct_msg(&regs, host_ctxt, HOST_FFA_ID);
		goto out_handled;
