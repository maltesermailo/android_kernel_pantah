...
DEFINE_PER_CPU(struct hyp_ftrace_stack, __ftrace_saved_frames);
				       ...
				       u32 *insn, void *__args)
{
	...
	if (__kern_addr(args->offset_idx, *func) != funcs_pg_func(*args->funcs_pg)) {
		ret = -EINVAL;
		goto end;
	}
			     ...
			     void *args)
{
	...
	while (funcs < funcs_end) {
		...
		asm volatile("str %w0, [%1]" : : "r" (insn), "r" ((u32 *)map));
					 ...
					 unsigned long offset_idx)
{
	...
	while (funcs < funcs_end) {
		if (__kern_addr(offset_idx, *funcs) == funcs_pg_func(host_func))
			return funcs;
		      ...
		      void *tramp)
{
	struct __ftrace_sync_patch_args args = {
		.tramp = tramp ? tramp : __hyp_ftrace_tramp,
