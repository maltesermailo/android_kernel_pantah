...
void *fixmap_map_slot(struct hyp_fixmap_slot *slot, phys_addr_t phys)
{
...
static int create_fixblock(void)
{
	...
	ret = pkvm_alloc_private_va_range(PMD_SIZE, &addr);
	if (ret)
		return ret;

	size = ALIGN(addr, PMD_SIZE) - addr;
	if (size) {
		unsigned long __addr;

		ret = pkvm_alloc_private_va_range(size, &__addr);
		if (ret)
			return ret;
