...
#define PKVM_PTDUMP_LOG_PAGES	1000
...
static int pkvm_ptdump_alloc_page(struct pkvm_ptdump_log_hdr **log_pages)
{
	struct pkvm_ptdump_log_hdr *p, *it;

	p = (void *)__get_free_page(GFP_KERNEL_ACCOUNT);
	if (!p)
		return -ENOMEM;
...
retry_dump:
	ret = kvm_call_hyp_nvhe(__pkvm_ptdump_handle, kvm->arch.pkvm.handle,
				PKVM_PTDUMP_WALK_RANGE, 0, -1, st->log_pages);
	if (ret == -ENOMEM) {
		for (int i = 0; i < PKVM_PTDUMP_LOG_PAGES; i++)
			pkvm_ptdump_alloc_page(&st->log_pages);
		goto retry_dump;
	} else if (ret != 0)
		return ret;
