 ...
 */
void *kvm_iommu_alloc_pages(void *gfp_flags_p, unsigned long order)
{
	return iommu_alloc_pages_sz(*(gfp_t *)gfp_flags_p, PAGE_SIZE << order);
}

/*
 * Must only be used with pages allocated via __pkvm_topup_hyp_iommu(), since that invokes
 * iommu_alloc_pages_sz().
 */
void kvm_iommu_free_pages(void *virt, void *arg, unsigned long order)
{
...
int __pkvm_topup_hyp_iommu(unsigned long nr_pages, unsigned long sz_alloc, gfp_t gfp)
{
	...
	ret = __topup_hyp_memcache(&mc, nr_pages, kvm_iommu_alloc_pages, kvm_host_pa, &gfp, order);
	if (ret)
		return ret;
	...
	if (ret) {
		...
		__free_hyp_memcache(&mc, kvm_iommu_free_pages, kvm_host_va, &mc);
	}
