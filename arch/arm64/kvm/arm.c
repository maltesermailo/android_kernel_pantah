...
static int init_pkvm_host_fp_state(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		kvm_nvhe_sym(kvm_arm_hyp_host_fp_state)[cpu] = (unsigned long)page_addr;
	}

	/* Map the pages in hyp. */
	for_each_possible_cpu(cpu) {
		char *begin = (char *)kvm_nvhe_sym(kvm_arm_hyp_host_fp_state)[cpu];
		char *end = begin + pkvm_host_fp_state_size();
		int ret;

		ret = create_hyp_mappings(begin, end, PAGE_HYP);
		if (ret) {
			kvm_err("Cannot map hyp host fp region\n");
			return ret;
		}
	}
