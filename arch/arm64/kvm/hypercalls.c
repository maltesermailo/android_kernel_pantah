...
static void kvm_sched_get_cpufreq(struct kvm_vcpu *vcpu, u64 *val)
{
...
static void kvm_sched_set_util(struct kvm_vcpu *vcpu, u64 *val)
{
	struct sched_attr attr = {
		.sched_flags = SCHED_FLAG_UTIL_HINT,
	};
	int ret;

	attr.sched_util = smccc_get_arg1(vcpu);
...
int kvm_hvc_call_handler(struct kvm_vcpu *vcpu)
{
	...
	case ARM_SMCCC_VENDOR_HYP_KVM_FEATURES_FUNC_ID:
		...
		val[0] |= BIT(ARM_SMCCC_KVM_FUNC_UTIL);
		val[0] |= BIT(ARM_SMCCC_KVM_FUNC_GET_CPUFREQ);
		val[0] |= BIT(ARM_SMCCC_KVM_FUNC_GET_CPUFREQ_TBL);
		break;
	case ARM_SMCCC_VENDOR_HYP_KVM_PTP_FUNC_ID:
