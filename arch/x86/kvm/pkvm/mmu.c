...
{
	if (size == 0 || PAGE_ALIGN(addr + size) <= PAGE_ALIGN_DOWN(addr))
		return false;
...
int pkvm_guest_mmu_refill_memcache(struct pkvm_vcpu *pkvm_vcpu)
{
	 ...
	 */
	struct pkvm_memcache tmp_mc = {0};
	int ret;

	host_mc = &pkvm_vcpu->shared_vcpu->arch.pkvm.guest_mmu_memcache;

	memcpy(&tmp_mc, host_mc, sizeof(struct pkvm_memcache));

	ret = refill_memcache(&vcpu->arch.pkvm.guest_mmu_memcache, tmp_mc.count, &tmp_mc);

	memcpy(host_mc, &tmp_mc, sizeof(struct pkvm_memcache));
