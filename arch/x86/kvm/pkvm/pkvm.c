...
/*
 * kvm_vcpu_sz represents the actual kvm_vcpu structure size.
...
static int pkvm_enable_virtualization_cpu(unsigned long pv_param_gpa)
{
	...
	/*
	 * The host can follow the way of sharing struct kvm and kvm_vcpu by
	 ...
	 *
	 * Alternatively, ensure the pv_param memory as PAGE_SIZE aligned and
	 * let the pkvm hypervisor to proactively share/unshare the pv_param
...
static void pkvm_disable_virtualization_cpu(void)
{
	...
	__pkvm_host_unshare_hyp(pkvm_virt_to_host_gpa(pv_param_va), size);
}
...
unpin:
	__pkvm_unpin_shared_mem(__pkvm_pa(shared_kvm), kvm_x86_call(vm_size));
undonate:
			      ...
			      unsigned long p4, unsigned long p5)
{
	...
	case __pkvm__host_unshare_hyp:
		ret = __pkvm_host_unshare_hyp(PFN_PHYS(p1), p2 * PAGE_SIZE);
		break;
	default:
