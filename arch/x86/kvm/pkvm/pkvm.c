...
static int pkvm_set_segment(struct kvm_vcpu *vcpu, struct kvm_segment *var, u32 seg)
{
	if (seg >= NR_VCPU_SEGMENTS)
		return -EINVAL;
...
static int pkvm_get_segment(struct kvm_vcpu *vcpu, struct kvm_segment *var, u32 seg)
{
	if (seg >= NR_VCPU_SEGMENTS)
		return -EINVAL;
...
static int pkvm_get_segment_base(struct kvm_vcpu *vcpu, u32 seg, u64 *data)
{
	if (seg >= NR_VCPU_SEGMENTS)
		return -EINVAL;
