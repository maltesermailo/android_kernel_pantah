...
static int setup_vcpu_lapic(struct kvm_vcpu *vcpu)
{
	...
	if (!pkvm_is_protected_vcpu(vcpu) || !enable_apicv) {
		...
		/*
		 * Sanity check to make sure the separate page for the vCPU's
		 * LAPIC registers is within the donated memory range of
		 * pkvm_vcpu.
		 */
		if ((apic->regs + PAGE_SIZE) > ((void *)pkvm_vcpu + pkvm_vcpu->size)) {
			pkvm_host_unshare_hyp(__pkvm_pa(shared_lapic_regs), PAGE_SIZE);
			ret = -ENOSPC;
			goto unshare_apic;
		}
	}
				     ...
				     struct pkvm_memcache *mc)
{
	...
	if (pkvm_is_protected_vcpu(vcpu)) {
		 ...
		 * in the guest CPUID. So if the guest CPUID doesn't have the
		 * x2apic feature, the x2apic enabling will be failed.
		 */
		u64 apic_base = APIC_DEFAULT_PHYS_BASE | LAPIC_MODE_X2APIC |
				(kvm_vcpu_is_reset_bsp(vcpu) ? MSR_IA32_APICBASE_BSP : 0);

		if (kvm_apic_set_base(vcpu, apic_base, true)) {
			vcpu->arch.cpuid_entries = old;
			vcpu->arch.cpuid_nent = old_nent;
			ret = -EINVAL;
			goto undonate;
		}
	}
