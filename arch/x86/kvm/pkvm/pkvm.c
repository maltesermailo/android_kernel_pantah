...
static struct pkvm_vcpu *detach_pkvm_vcpu_from_vm(struct pkvm_vm *pkvm_vm, int vcpu_handle)
{
	...
	if (refcount > 1) {
		...
		pr_err("VM%d vcpu%d is busy, refcount %d\n",
...
static void pkvm_vm_destroy(int handle)
{
	...
	/*
	 * Suppose all the created pkvm_vcpus have been freed already by the
	 * host via the vcpu_free PV interface. In case any of the pkvm_vcpu
	 * is not freed, try to free it at here.
	 */
...
static void pkvm_vcpu_load(struct pkvm_vcpu *pkvm_vcpu, int cpu)
{
	...
	if (loaded_cpu == -1) {
		/*
		 * Get to prevent the pkvm_vcpu from being freed via the
		 * vcpu_free PV interface without a pkvm_vcpu_put. If the
		 * got pkvm_vcpu is not the same with the original pkvm_vcpu, it
		 * must be a code bug.
		 */
