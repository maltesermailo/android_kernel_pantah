...
static __init int pkvm_setup_host_vmcs_config(void)
{
	...
	if (boot_cpu_has(X86_FEATURE_INTEL_PT)) {
		/*
		 * Enable the RTIT vmexit/vmentry controls to guarantee the
	...
	if (boot_cpu_has(X86_FEATURE_ARCH_LBR)) {
		/*
		 * Enable the arch-LBR vmexit/vmentry controls to guarantee the
	...
	if (boot_cpu_has(X86_FEATURE_IBT) || boot_cpu_has(X86_FEATURE_SHSTK)) {
		...
		/*
		 * Enable the CET vmexit/vmentry controls to guarantee the
