					...
					ioasid_t pasid, enum cache_tag_type type)
{
	...
	pkvm_dbg("pkvm: %s: cache_assign: dev[%x] did:%u, pasid: %u,"
		 "ats_qdep: %u, type: %s\n",
	...
	list_for_each_entry(cache_tag, &domain->cache_tags, node) {
		if (cache_tag_match(cache_tag, did, iommu, bus, devfn, pasid, type)) {
			if (--cache_tag->users == 0) {
				pkvm_dbg("pkvm: %s: cache_unassign: dev[%x] did:%u, pasid: %u, "
					 "type: %s\n", __func__, PCI_DEVID(bus, devfn), did,
					 pasid, type == CACHE_TAG_IOTLB ? "IOTLB" : "DEVTLB");
