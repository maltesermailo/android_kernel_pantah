...
retry:
	...
	if (!entries) {
		...
		ret = __pkvm_host_donate_hyp_share_ro(*ptable_gpa, VTD_PAGE_SIZE);
		if (ret) {
		...
		if (!try_cmpxchg64(&dir[dir_index].val, &tmp,
			...
			ret = __pkvm_hyp_donate_host_unshare_ro(*ptable_gpa, VTD_PAGE_SIZE);
			if (ret) {
...
int pkvm_iommu_clear_pasid_entry(u64 param_va)
{
	...
	pkvm_dbg("pkvm: %s: clear_pe: dev[%x] pasid: %u, did: %u, pgd: %llx\n",
...
out_unlock:
	...
	if (is_dev_in_satc(param->bdf) && param->ats_enabled) {
		u16 pfsid = 0;
