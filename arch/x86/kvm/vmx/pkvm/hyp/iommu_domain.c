	...
	while (1) {
		...
		if (!dma_pte_present(pte)) {
			...
			if (!try_cmpxchg64(&pte->val, &tmp, pteval)) {
				...
				__pkvm_hyp_donate_host(donation->pages[donation->nr_pages], PAGE_SIZE);
				donation->nr_pages++;
				PKVM_ASSERT(donation->nr_pages <= PKVM_MAX_IOMMU_PAGE_DONATION);
			} else {
...
static inline int verify_page_onwership(unsigned long phys_start, int level)
{
	...
	phys_end = phys_start + level_size(level) - 1;
	phys_start += level_size(ept_level);
...
static inline void decrease_page_ref(unsigned long addr, int level)
{
	...
	for (phys = addr; phys < addr + level_size(level); phys += VTD_PAGE_SIZE)
