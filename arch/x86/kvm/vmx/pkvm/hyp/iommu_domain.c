...
static struct pkvm_iommu_domain *pkvm_alloc_iommu_domain(u64 pgd)
{
...
struct pkvm_iommu_domain *pkvm_domain_attach_device(u64 pgd, u16 bdf, bool iommu_coherency)
{
	struct pkvm_iommu_domain *domain;
	...
	if (!ptdev) {
		ptdev = pkvm_alloc_ptdev(bdf, iommu_coherency);
		if (!ptdev)
			return NULL;
	}
	...
	domain->iommu_coherency = iommu_coherency;
...
static inline struct pkvm_ptdev *__domain_find_ptdev_locked(struct pkvm_iommu_domain *domain, u16 bdf)
{
	struct pkvm_ptdev *ptdev;

	list_for_each_entry(ptdev, &domain->ptdev_head, domain_node) {
...
struct pkvm_ptdev *pkvm_domain_find_device(struct pkvm_iommu_domain *domain, u16 bdf)
{
	struct pkvm_ptdev *ptdev;
	pkvm_spin_lock(&domain->lock);
	ptdev = __domain_find_ptdev_locked(domain, bdf);
	pkvm_spin_unlock(&domain->lock);

	return ptdev;
}

int pkvm_domain_detach_device(struct pkvm_iommu_domain *domain, u16 bdf)
{
	struct pkvm_ptdev *ptdev;
				      ...
				      unsigned long pfn, int *target_level)
{
	...
	while (1) {
		...
		if (!dma_pte_present(pte)) {
			...
			if (pkvm_switch_host_ept_ro(donation->pages[donation->nr_pages], PAGE_SIZE)) {
			...
			if (!try_cmpxchg64(&pte->val, &tmp, pteval)) {
				...
				pkvm_switch_host_ept_default(donation->pages[donation->nr_pages], PAGE_SIZE);
				donation->nr_pages++;
			       ...
			       unsigned long last_pfn)
{
	...
	do {
		...
		if (level < retain_level && !(start_pfn > level_pfn ||
			...
			dma_clear_pte(pte);
			domain_flush_cache(domain, pte, sizeof(*pte));
			memset(level_pte, 0, PAGE_SIZE);
			if (pkvm_switch_host_ept_default(virt_to_phys(level_pte), PAGE_SIZE)) {
				   ...
				   int retain_level)
{
	...
	/*
	 * TODO: Reviw this code. This 'if' seems buggy and unnecessary.
	 */
	if (start_pfn == 0 && last_pfn == DOMAIN_MAX_PFN(domain->gaw)) {
		donation->pages[donation->nr_pages++] = domain->pgd;
		PKVM_ASSERT(donation->nr_pages <= PKVM_MAX_IOMMU_PAGE_DONATION);
		pkvm_switch_host_ept_default(domain->pgd, PAGE_SIZE);
		domain->pgd = 0ULL;
	}
}
...
unsigned long pkvm_iommu_domain_map(unsigned long param_gpa,
				unsigned long donation_gpa)
{
	struct pkvm_iommu_page_donation *donation;
	...
	donation = host_gpa2hva(donation_gpa);
...
out:
	memset(pkvm_phys_to_virt(phys), 0, PAGE_SIZE);
	if (pkvm_switch_host_ept_default(phys, PAGE_SIZE)) {
		pkvm_err("pkvm: %s: failed to remove write protect page!\n", __func__);
		return;
	}
