				      ...
				      unsigned long pfn, int *target_level)
{
	...
	while (1) {
		...
		if (!dma_pte_present(pte)) {
			...
			if (__pkvm_host_donate_hyp_request_ro(donation->pages[donation->nr_pages], PAGE_SIZE)) {
			...
			if (!try_cmpxchg64(&pte->val, &tmp, pteval)) {
				...
				__pkvm_hyp_release_host(donation->pages[donation->nr_pages], PAGE_SIZE);
				donation->nr_pages++;
			       ...
			       unsigned long last_pfn)
{
	...
	do {
		...
		if (level < retain_level && !(start_pfn > level_pfn ||
			...
			if (__pkvm_hyp_release_host(virt_to_phys(level_pte), PAGE_SIZE)) {
...
out:
	...
	if (__pkvm_hyp_release_host(phys, PAGE_SIZE)) {
		pkvm_err("pkvm: %s: failed to remove write protect page!\n", __func__);
		return;
	}
		...
		unsigned long last_pfn, bool dma_strict_mode)
{
	...
	if (dma_strict_mode) {
		...
		unsigned long end = (last_pfn << VTD_PAGE_SHIFT) + 1;
		/*
