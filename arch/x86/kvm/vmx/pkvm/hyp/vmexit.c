...
static unsigned long handle_vmcall(struct kvm_vcpu *vcpu)
{
	...
	case PKVM_HC_DISABLE_IOMMU:
		...
		break;
	case PKVM_HC_IOMMU_CLEAR_CE:
		ret = pkvm_iommu_clear_ce(a0, a1);
		break;
	case PKVM_HC_IOMMU_SET_LM_CE:
		ret = pkvm_iommu_set_lm_ce(a0, a1);
		break;
	case PKVM_HC_IOMMU_SET_LM_PTCE:
		ret = pkvm_iommu_set_lm_ptce(a0, a1);
		break;
	...
	case PKVM_HC_IOMMU_CLEAR_PASID_ENTRY:
		...
		break;
	case PKVM_HC_IOMMU_SET_PASID_FL:
		ret = pkvm_iommu_pasid_setup_fl(a0, a1);
		break;
	case PKVM_HC_IOMMU_SET_PASID_SL:
		ret = pkvm_iommu_pasid_setup_sl(a0, a1);
		break;
	case PKVM_HC_IOMMU_SET_PASID_FT:
		ret = pkvm_iommu_pasid_setup_pt(a0, a1);
		break;
	case PKVM_HC_IOMMU_DOMAIN_ATTACH:
		ret = pkvm_iommu_domain_attach(a0, a1);
		break;
	case PKVM_HC_IOMMU_DOMAIN_DETACH:
		ret = pkvm_iommu_domain_detach(a0, a1, a2);
		break;
