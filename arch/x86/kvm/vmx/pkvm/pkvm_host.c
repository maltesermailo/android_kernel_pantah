 ...
 *
 * Note: The pkvm hypervisor also supports boot time retpoline/rethunk patching
 * to mitigate certain CPU vulnerabilities, as it is part of linux kernel, and
 * linux kernel could turn on this mitigation with kernel command line parameter
 * to patch the kernel image including the pkvm hypervisor at the boot time even
 * such CPU vulnerabilities don't exist on PTL. Otherwise the pkvm hypervisor
 * will be broken.
 */
