...
#define PKVM_HC_IOMMU_CLEAR_PASID_ENTRY	17
#define PKVM_HC_IOMMU_SET_PASID_FL	18
#define PKVM_HC_IOMMU_SET_PASID_SL	19
#define PKVM_HC_IOMMU_SET_PASID_FT	20
#define PKVM_HC_IOMMU_SET_SM_CE_PRE	21
#define PKVM_HC_IOMMU_DOMAIN_ATTACH	22
#define PKVM_HC_IOMMU_DOMAIN_DETACH	23
#define PKVM_HC_IOMMU_MAP_PAGES		24
#define PKVM_HC_IOMMU_UNMAP_PAGES	25
#define PKVM_HC_IOMMU_CACHE_ASSIGN	26
#define PKVM_HC_IOMMU_CACHE_UNASSIGN	27
#define PKVM_HC_SUBMIT_QI		28
 ...
 * entries.
 */
struct pkvm_block_translation_param {
	/*
...
struct pkvm_cache_assign_param {
	u64 pgd_gpa;
	int type;
	u64 phys;
	u8 bus;
		...
		struct pkvm_block_translation_param *param)
{
	...
	if (pkvm_enabled())
		ret = kvm_hypercall2(PKVM_HC_IOMMU_CLEAR_CE, reg_phys, virt_to_phys(param));
...
static inline long pkvm_hc_iommu_set_lm_ptce(unsigned long reg_phys,
		struct pkvm_context_param *param)
{
	long ret = 0;
...
static inline long pkvm_hc_iommu_set_pasid_pt(unsigned long reg_phys,
		struct pkvm_pasid_table_param *param)
{
	long ret = 0;

	if (pkvm_enabled())
		ret = kvm_hypercall2(PKVM_HC_IOMMU_SET_PASID_FT, reg_phys,
				virt_to_phys(param));

	return ret;
}

static inline long pkvm_hc_iommu_domain_attach(unsigned long reg_phys,
		struct pkvm_domain_param *param)
{
	long ret = 0;

	if (pkvm_enabled())
		ret = kvm_hypercall2(PKVM_HC_IOMMU_DOMAIN_ATTACH, reg_phys,
				virt_to_phys(param));

	return ret;
}

static inline long pkvm_hc_iommu_domain_detach(unsigned long reg_phys,
		u64 pgd_gpa, u16 bdf)
{
	long ret = 0;

	if (pkvm_enabled())
		ret = kvm_hypercall3(PKVM_HC_IOMMU_DOMAIN_DETACH, reg_phys, pgd_gpa, bdf);
