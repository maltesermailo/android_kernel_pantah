...
static inline long pkvm_iommu_map_pages(struct pkvm_iommu_map_param *param)
{
	long ret = 0;
	if (likely(this_cpu_read(pkvm_enabled))) {
		ret = kvm_hypercall1(PKVM_HC_IOMMU_MAP_PAGES, (unsigned long)param);
	}
	return ret;
}

/*
 * parameters passed by host for UNMAP_PAGE hypercall.
 */
struct pkvm_iommu_unmap_param {
	u64 pgd_gpa;
	u64 start_pfn;
	u64 last_pfn;
	struct pkvm_iommu_page_donation donation;
...
static inline long pkvm_iommu_unmap_pages(struct pkvm_iommu_unmap_param *param)
{
	long ret = 0;
	if (likely(this_cpu_read(pkvm_enabled))) {
		ret = kvm_hypercall1(PKVM_HC_IOMMU_UNMAP_PAGES, (unsigned long)param);
	}
	return ret;
}
