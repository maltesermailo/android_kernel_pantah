...
fn kstrtobool(kstr: &CStr) -> Result<bool> {
    let mut res = false;
    to_result(unsafe { bindings::kstrtobool(kstr.as_char_ptr(), &mut res) })?;
    Ok(res)
}
...
unsafe extern "C" fn unpin_store(
    ...
    let unpinning_enable = match kstrtobool(buf) {
        Ok(unpinning_enable) => unpinning_enable,
        Err(err) => return err.to_errno() as isize,
    };

    match ashmem_range::set_shrinker_enabled(unpinning_enable) {
        Ok(()) => count as isize,
