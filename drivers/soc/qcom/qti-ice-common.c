...
static int qti_ice_check_fuse_setting(struct qti_ice_entry *ice_entry)
{
	uint32_t regval;
...
static int qti_ice_check_version(struct qti_ice_entry *ice_entry)
{
	...
	if (major < ICE_CORE_CURRENT_MAJOR_VERSION) {
		pr_err("%s: Unknown ICE device at %lu, rev %d.%d.%d\n",
			__func__, (unsigned long)ice_entry->ice_mmio,
				major, minor, step);
		return -ENODEV;
	}
...
int qti_ice_init_crypto(struct device *dev, void __iomem *mmio,
			   void **priv_data)
{
	int err = 0;
	...
	ice_entry = devm_kzalloc(dev,
		sizeof(struct qti_ice_entry),
		GFP_KERNEL);
	if (!ice_entry)
		return -ENOMEM;

	ice_entry->ice_mmio = mmio;
	ice_entry->flags = 0;

	err = qti_ice_check_version(ice_entry);
	if (err) {
		pr_err("%s: check version failed, err %d\n", __func__, err);
		return err;
	}

	err = qti_ice_check_fuse_setting(ice_entry);
	if (err)
		return err;
...
EXPORT_SYMBOL(qti_ice_init_crypto);
...
static int qti_ice_wait_bist_status(struct qti_ice_entry *ice_entry)
{
	int count;
	uint32_t regval;
...
static void qti_ice_enable_intr(struct qti_ice_entry *ice_entry)
{
	uint32_t regval;
...
static void qti_ice_disable_intr(struct qti_ice_entry *ice_entry)
{
	uint32_t regval;
...
int qti_ice_enable(void *priv_data)
{
	int err = 0;
	struct qti_ice_entry *ice_entry;

	ice_entry = (struct qti_ice_entry *) priv_data;
	if (!ice_entry) {
		pr_err("%s: vops ice data is invalid\n", __func__);
		return -EINVAL;
	}

	qti_ice_low_power_and_optimization_enable(ice_entry);
	err = qti_ice_wait_bist_status(ice_entry);
	if (err)
		return err;
	qti_ice_enable_intr(ice_entry);

	return err;
}
EXPORT_SYMBOL(qti_ice_enable);

void qti_ice_disable(void *priv_data)
{
	struct qti_ice_entry *ice_entry;
	...
	qti_ice_disable_platform(ice_entry);
	qti_ice_disable_intr(ice_entry);
}
EXPORT_SYMBOL(qti_ice_disable);

int qti_ice_resume(void *priv_data)
{
	int err = 0;
	struct qti_ice_entry *ice_entry;
...
static void qti_ice_dump_test_bus(struct qti_ice_entry *ice_entry)
{
	uint32_t regval = 0x1;
	uint32_t val;
...
int qti_ice_debug(void *priv_data)
{
	struct qti_ice_entry *ice_entry;
	...
	qti_ice_dump_test_bus(ice_entry);

	return 0;
}
EXPORT_SYMBOL(qti_ice_debug);

int qti_ice_keyslot_program(void *priv_data,
			       ...
			       u8 data_unit_mask, int capid)
{
	...
	err = qti_ice_program_key(ice_entry, key, slot,
				data_unit_mask, capid);
	if (err) {
		pr_err("%s: program key failed with error %d\n", __func__, err);
		err = qti_ice_invalidate_key(ice_entry, slot);
		if (err) {
...
int qti_ice_keyslot_evict(void *priv_data, unsigned int slot)
{
	int err = 0;
	struct qti_ice_entry *ice_entry;
...
int qti_ice_derive_raw_secret(void *priv_data,
				 ...
				 unsigned int secret_size)
{
	...
	return qti_ice_derive_raw_secret_platform(ice_entry,
				wrapped_key, wrapped_key_size,
				secret, secret_size);
}
EXPORT_SYMBOL(qti_ice_derive_raw_secret);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("Common ICE library for storage encryption");
