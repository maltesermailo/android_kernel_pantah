...
static size_t count_dmabuf_fds(struct files_struct *files)
{
	...
	for (unsigned int fd = 0; fd < files_fdtable(files)->max_fds; ++fd) {
...
int dma_buf_begin_new_exec(struct files_struct *old_files)
{
	...
	if (my_files) {
		...
		num_dmabuf_fds = count_dmabuf_fds(current->files);
...
retry:
		...
		num_dmabuf_fds_check = count_dmabuf_fds(current->files);
