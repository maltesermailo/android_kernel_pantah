...
static void dma_buf_vma_open(struct vm_area_struct *vma)
{
	...
	if (dmabuf->heap_vm_ops->open)
		dmabuf->heap_vm_ops->open(vma);
}
...
static void dma_buf_vma_close(struct vm_area_struct *vma)
{
	...
	if (dmabuf->heap_vm_ops->close)
		dmabuf->heap_vm_ops->close(vma);
}
#endif
...
#ifdef CONFIG_DMABUF_SYSFS_STATS
	dmabuf->heap_vm_ops = vma->vm_ops;
	/* copy the heap provided vma ops */
	dmabuf->vm_ops = *(dmabuf->heap_vm_ops);
