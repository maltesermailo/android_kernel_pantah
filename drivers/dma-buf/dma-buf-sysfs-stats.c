...
static void sysfs_add_workfn(struct work_struct *work)
{
	...
	/*
	 * It served its purpose. Safe to free the work item from the callback
	 * as it is guaranteed that workqueue code does not access it after
...
int dma_buf_stats_setup(struct dma_buf *dmabuf)
{
	...
	create_entry = kzalloc(sizeof(struct dma_buf_create_sysfs_entry), GFP_KERNEL);
	if (!create_entry) {
		kfree(sysfs_entry);
		return -ENOMEM;
	}
