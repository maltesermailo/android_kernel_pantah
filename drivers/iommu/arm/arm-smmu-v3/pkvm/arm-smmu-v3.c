...
static void smmu_tlb_flush_all(void *cookie)
{
	...
	list_for_each_entry(iommu_node, &smmu_domain->iommu_list, list) {
		...
		hyp_spin_lock(kvm_hyp_iommu_cast_lock(&smmu->iommu.lock));
		if (smmu->iommu.power_is_off && smmu->caches_clean_on_power_on) {
			hyp_spin_unlock(&smmu->iommu.lock);
			continue;
		}
		WARN_ON(smmu_send_cmd(smmu, &cmd));
		hyp_spin_unlock(kvm_hyp_iommu_cast_lock(&smmu->iommu.lock));
	}
			   ...
			   u32 sid, u32 pasid, u32 pasid_bits)
{
	...
	hyp_spin_lock(kvm_hyp_iommu_cast_lock(&iommu->lock));
...
out_unlock:
	if (ret && iommu_node)
		hyp_free(iommu_node);
	hyp_spin_unlock(kvm_hyp_iommu_cast_lock(&iommu->lock));
