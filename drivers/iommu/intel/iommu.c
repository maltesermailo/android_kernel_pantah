			  ...
			  unsigned long long addr, u32 pasid)
{
	...
	if (!sm_supported(iommu)) {
		...
		if (pkvm_pviommu_enabled() &&
			 ...
			 * to DMA mapping and uses host ept which the host do
			 * not have access.
			 */
			pr_info("passthrough mode is shadowed and write protected by pkvm\n");
			return;
		}
				      ...
				      u8 bus, u8 devfn)
{
	...
	if (ret == -ENOMEM) {
		...
		ret = pkvm_hc_iommu_set_lm_ce(&param);
		/*
		 * if pkvm did not use the donated page, free it.
		 */
...
static void pv_context_clear(struct device_domain_info *info, u8 bus, u8 devfn)
{
	struct pkvm_clear_translation_param param = {
		...
		.bdf = PCI_DEVID(bus, devfn),
		.ats_enabled = info->ats_enabled,
		.ats_qdep = info->ats_qdep,
	};

	if (pkvm_hc_iommu_clear_ce(&param)) {
