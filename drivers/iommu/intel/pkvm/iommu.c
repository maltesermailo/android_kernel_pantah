 ...
 * When called during iommu_init(), the table would already be initialized and
 * host is trusted, so don't bother clearing the table.
 */
static int iommu_protect_ir_table(struct intel_iommu *iommu, bool clear)
{
	u64 ir_table_pa;
	int ret;
	...
	ret = pkvm_host_donate_hyp_share_ro(ir_table_pa, SZ_1M, clear);
	if (ret) {
...
static int handle_gcmd_sirtp(struct intel_iommu *iommu)
{
	...
	ret = iommu_protect_ir_table(iommu, true);
	if (ret)
		return ret;
...
static int iommu_init(struct intel_iommu *iommu)
{
	...
	if (iommu->vgsts & DMA_GSTS_IRTPS) {
		...
		ret = iommu_protect_ir_table(iommu, false);
		if (ret)
			return ret;
	}
