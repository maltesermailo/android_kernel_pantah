...
{
	char *serdev_dup, *driver_name, *port_name, *serdev_name;
	...
	serdev_dup = kstrdup(serdev, GFP_KERNEL);
	if (!serdev_dup)
		return -ENOMEM;

	driver_name = strsep(&serdev_dup, "/");
	if (!driver_name) {
		gnss_cmdline_err("driver name missing\n");
		goto err_free_serdev_dup;
	}

	port_name = strsep(&serdev_dup, "/");
	if (!port_name) {
		gnss_cmdline_err("port name missing\n");
		goto err_free_serdev_dup;
	}

	serdev_name = strsep(&serdev_dup, "/");
	if (!serdev_name) {
