...
static inline int ufshcd_mcq_enabled(struct ufs_hba *hba)
{
	...
	trace_android_vh_ufs_mcq_is_enabled(hba, &enabled);
...
start_window:
	...
	has_outstanding = (hba->outstanding_reqs != 0) ? true : false;
...
static inline int ufshcd_hba_capabilities(struct ufs_hba *hba)
{
	...
	if (ufshcd_mcq_enabled(hba)) {
		trace_android_rvh_ufs_mcq_hba_capabilities(hba, &err);
		return err;
	}
...
static int ufshcd_queuecommand(struct Scsi_Host *host, struct scsi_cmnd *cmd)
{
	...
	trace_android_vh_ufs_mcq_set_sqid(hba, scsi_cmd_to_rq(cmd)->mq_hctx->queue_num);
		...
		enum dev_cmd_type cmd_type, int timeout)
{
	...
	trace_android_vh_ufs_mcq_set_sqid(hba, 0);
...
static int ufshcd_memory_alloc(struct ufs_hba *hba)
{
	...
	if (ufshcd_mcq_enabled(hba)) {
		err = 0;
		trace_android_rvh_ufs_mcq_memory_alloc(hba, &err);
		return err;
	}
...
int ufshcd_init(struct ufs_hba *hba, void __iomem *mmio_base, unsigned int irq)
{
	...
	trace_android_vh_ufs_mcq_init_irq(hba);
