...
static void xe_gpufreqtracer_sample_work(struct work_struct *work)
{
	...
	if (current_freq != last_freq) {
		drm_dbg(&xe->drm, "GT%u frequency changed, tracing %u KHz (period %u sec)",
			gt->info.id, current_freq,
			atomic_read(&gt_data->gpu_active)
				? XE_GPUFREQ_MONITORING_MIN_INTERVAL_MS / 1000
				: xe_modparam.gpufreq_monitoring_interval_ms / 1000);
