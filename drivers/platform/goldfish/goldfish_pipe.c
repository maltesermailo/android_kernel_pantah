				    ...
				    size_t bufflen, int is_write)
{
	...
	while (address < address_end) {
		...
		pr_info("%s: requested pages: %d %ld %p\n", __func__, ret,
			requested_pages, (void*)first_page);
		if (ret == 0) {
			pr_info("%s: error: (requested pages == 0) (wanted %ld)\n",
					__func__, requested_pages);
			mutex_unlock(&pipe->lock);
			return ret;
		}
		if (ret < 0) {
			pr_info("%s: (requested pages < 0) %ld \n",
				 	__func__, requested_pages);
			mutex_unlock(&pipe->lock);
			return ret;
		}
		...
		for (page_i = 1; page_i < ret; page_i++) {
			...
			if (xaddr_i == xaddr_prev + PAGE_SIZE) {
				...
				pr_info("%s: discontinuous page boundary: %d pages instead\n",
						__func__, page_i);
				break;
			}
		}
...
static int goldfish_pipe_open(struct inode *inode, struct file *file)
{
	...
	pr_info("%s: call. pipe_dev dev=%p new_pipe_addr=%p file=%p\n",
