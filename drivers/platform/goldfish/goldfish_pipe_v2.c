...
{
	pr_info("PIPE_WAKE_UNLOCK_DMA: unlock pipe dma for pipe 0x%p\n", pipe);
	if (!pipe->closing && pipe->dma) {
		WARN_ON(!pipe->dma->locked);
		clear_bit(BIT_WAKE_ON_UNLOCK_DMA, &pipe->flags);
		pipe->dma->locked = false;
		/* Meant to be used with wake_up_interruptible --
		 * otherwise no signaling,
...
static void goldfish_pipe_dma_release_host_locked(struct goldfish_pipe *pipe)
{
	...
	pr_debug("after delete of dma @ 0x%llx: alloc total %llu\n",
...
static int goldfish_pipe_release(struct inode *inode, struct file *filp)
{
	...
	if (pipe->dma)
		mutex_lock(&pipe->dma->mutex_lock);
	...
	struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GOLDFISH_DMA_IOC_CREATE_REGION:
		...
		if (!is_page_size_multiple(ioctl_data.size)) {
			pr_err("DMA_CREATE_REGION: %llu not a multiple "
				"of page size!\n",
				ioctl_data.size);
			return -EINVAL;
		}
