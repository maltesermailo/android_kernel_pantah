...
static int mv2220_suspend(struct phy_device *phydev)
{

	int phydata_read = 0;
	struct mii_bus *bus = phydev->mdio.bus;

	phydata_read = bus->read(bus, MV88Q2220_PMA_PMD_REG_ADDR , 0x0900);
	phydata_read = phydata_read | 0x800;
	bus->write(bus, MV88Q2220_PMA_PMD_REG_ADDR , 0x0900, phydata_read);

	return 0;
}

static int mv2220_resume(struct phy_device *phydev)
{
	...
	phydata_read = bus->read(bus, MV88Q2220_PMA_PMD_REG_ADDR , 0x0900);
	phydata_read = phydata_read & 0xF7FF;
	bus->write(bus, MV88Q2220_PMA_PMD_REG_ADDR , 0x0900, phydata_read);
...
{
	int err;
pr_info("qcom-ethqos mv2220_probe in msm-kernel");
...
static struct phy_driver marvell_drivers[] = {
	...
	},
	{
		...
		.suspend = &mv2220_suspend,
		.resume = &mv2220_resume,
	},
};
