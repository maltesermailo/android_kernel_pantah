...
hwsim_ftm_result_policy[NL80211_PMSR_FTM_RESP_ATTR_MAX + 1] = {
	...
	[NL80211_PMSR_FTM_RESP_ATTR_RX_RATE] =
		NLA_POLICY_NESTED(hwsim_rate_info_policy),
	[NL80211_PMSR_FTM_RESP_ATTR_RTT_AVG] = { .type = NLA_S64 },
	[NL80211_PMSR_FTM_RESP_ATTR_RTT_VARIANCE] = { .type = NLA_S64 },
	[NL80211_PMSR_FTM_RESP_ATTR_RTT_SPREAD] = { .type = NLA_S64 },
	[NL80211_PMSR_FTM_RESP_ATTR_DIST_AVG] = { .type = NLA_S64 },
	[NL80211_PMSR_FTM_RESP_ATTR_DIST_VARIANCE] = { .type = NLA_S64 },
	[NL80211_PMSR_FTM_RESP_ATTR_DIST_SPREAD] = { .type = NLA_S64 },
...
static int mac80211_hwsim_send_pmsr_ftm_request_peer(
		struct sk_buff *msg,
...
static int mac80211_hwsim_send_pmsr_request(
		struct sk_buff *msg,
				     ...
				     struct cfg80211_pmsr_request *request)
{
	...
	printk(KERN_ERR "jaewan: mac80211_hwsim start pmsr #1\n");
	...
	printk(KERN_ERR "jaewan: mac80211_hwsim start pmsr #2\n");
	...
	printk(KERN_ERR "jaewan: mac80211_hwsim start pmsr #3\n");
	...
	printk(KERN_ERR "jaewan: mac80211_hwsim start pmsr #4\n");
	...
	printk(KERN_ERR "jaewan: mac80211_hwsim start pmsr #5\n");

	err = mac80211_hwsim_send_pmsr_request(skb, request);
	if (err)
		goto out_err;

	printk(KERN_ERR "jaewan: mac80211_hwsim start pmsr #6\n");

	nla_nest_end(skb, pmsr);

	genlmsg_end(skb, msg_head);
	printk(KERN_ERR "jaewan: mac80211_hwsim start pmsr #7\n");
	if (hwsim_virtio_enabled)
	{printk(KERN_ERR "jaewan: mac80211_hwsim start pmsr #7-1\n");
		hwsim_tx_virtio(data, skb);
	}
	else
	{printk(KERN_ERR "jaewan: mac80211_hwsim start pmsr #7-2\n");
		hwsim_unicast_netgroup(data, skb, _portid);
	}
					   ...
					   struct genl_info *info)
{
	...
	ret = nla_parse_nested_deprecated(tb, HWSIM_RATE_INFO_ATTR_MAX,
					   ...
					   struct genl_info *info)
{
	...
	ret = nla_parse_nested_deprecated(tb, NL80211_PMSR_FTM_REQ_ATTR_MAX,
					  ...
					  struct genl_info *info)
{
	...
	ret = nla_parse_nested_deprecated(tb, NL80211_PMSR_RESP_ATTR_MAX, resp,
	...
	if (tb[NL80211_PMSR_RESP_ATTR_DATA]) {
		printk(KERN_ERR "jaewan: hwsim: mac80211_hwsim_parse_pmsr_resp .. has ftm..\n");

		nla_for_each_nested(pmsr, tb[NL80211_PMSR_RESP_ATTR_DATA], rem) {
			switch (nla_type(pmsr)) {
			case NL80211_PMSR_TYPE_FTM:
				result->type = NL80211_PMSR_TYPE_FTM;
				ret = mac80211_hwsim_parse_ftm_result(
					pmsr, &result->ftm, info);
				if (ret)
					return ret;
				break;
			default:
				printk(KERN_ERR "jaewan: hwsim: mac80211_hwsim_parse_pmsr_resp ... unknown\n");
					    ...
					    struct genl_info *info)
{
	...
	if (!peer)
	{printk(KERN_ERR "jaewan: hwsim: mac80211_hwsim_parse_pmsr_result: empty peer\n");
		return -EINVAL;
	}

	ret = nla_parse_nested_deprecated(tb, NL80211_PMSR_PEER_ATTR_MAX, peer,
					  hwsim_pmsr_peer_result_policy,
					  info->extack);
	if (ret)
	{printk(KERN_ERR "jaewan: hwsim: mac80211_hwsim_parse_pmsr_result failed to parse '1'\n");
		return ret;
	}
	...
	if (tb[NL80211_PMSR_PEER_ATTR_RESP]) {
		printk(KERN_ERR "jaewan: hwsim: mac80211_hwsim_parse_pmsr_result: yes resp\n");
		ret = mac80211_hwsim_parse_pmsr_resp(
			tb[NL80211_PMSR_PEER_ATTR_RESP], result, info);
		if (ret)
			return ret;
	}

printk(KERN_ERR "jaewan: hwsim: mac80211_hwsim_parse_pmsr_result: done\n");
	return 0;
};
...
static int hwsim_pmsr_report_nl(struct sk_buff *msg, struct genl_info *info)
{
	...
	if (!reqattr) {
		printk(KERN_ERR "jaewan: hwsim: hwsim_pmsr_report_nl: empty reqattr\n");
		err = -EINVAL;
		goto out_err;
	}
	...
	if (!peers) {
		printk(KERN_ERR "jaewan: hwsim: hwsim_pmsr_report_nl: empty peers\n");
		err = -EINVAL;
		goto out_err;
	}
...
static int hwsim_virtio_handle_cmd(struct sk_buff *skb)
{
	...
	if (extack._msg) printk(KERN_ERR "jaewan: hwsim: unprinted err %s\n", extack._msg);
