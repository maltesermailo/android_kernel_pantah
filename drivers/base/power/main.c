...
Run:
	error = dpm_run_callback(callback, dev, state, info);
	if (error) {
<<<<<<< HEAD   (c47064085b01b8e8acc3d8ecc51b3d985f2d37f1 Revert "ANDROID: perf: Export clock_set_rate tracepoint")
		async_error = error;
		log_suspend_abort_reason("Device %s failed to %s noirq: error %d",
					 dev_name(dev), pm_verb(state.event), error);
||||||| BASE   (ae388edd4a8f0226f3ef7b102c34f78220756c3d Merge tag 'landlock-6.17-rc1' of git://git.kernel.org/pub/sc)
		async_error = error;
=======
		WRITE_ONCE(async_error, error);
>>>>>>> BRANCH (53edfecef66bfa65882ae065ed1a52f466c88979 Merge tag 'pm-6.17-rc1' of git://git.kernel.org/pub/scm/linu)
		dpm_save_failed_dev(dev_name(dev));
		pm_dev_err(dev, state, async ? " async noirq" : " noirq", error);
		goto Complete;
	}
