...
static int trusty_test_share_obj(struct trusty_test_state *s,
				 struct trusty_test_shmem_obj *obj)
{
	return trusty_share_memory(s->trusty_dev, &obj->mem_id, obj->sgt.sgl,
				   obj->sgt.nents, PAGE_KERNEL);
}

static int trusty_test_revoke_obj(struct trusty_test_state *s,
				  struct trusty_test_shmem_obj *obj)
{
	return trusty_revoke_memory(s->trusty_dev, obj->mem_id, obj->sgt.sgl,
				    obj->sgt.nents);
}
			   ...
			   size_t repeat_access)
{
	...
	for (i = 0; i < repeat_share && !ret; i++) {
		list_for_each_entry_safe(obj, next_obj, &obj_list, node) {
			t1 = ktime_get();
			tmpret = trusty_test_share_obj(s, obj);
			t2 = ktime_get();
			if (tmpret) {
				ret = tmpret;
				dev_err(s->dev, "trusty_share_memory failed: %d, i=%zd/%zd, size=%zd\n",
		...
		if (!ret) {
			...
			if (ret)
				dev_err(s->dev, "test failed: %d, i=%zd/%zd, size=%zd\n",
					ret, i, repeat_share, size);
		}

		list_for_each_entry_safe(obj, next_obj, &obj_list, node) {
			t1 = ktime_get();
			tmpret = trusty_test_revoke_obj(s, obj);
			t2 = ktime_get();
			if (tmpret) {
				ret = tmpret;
				dev_err(s->dev,
					"trusty_revoke_memory failed: %d, id=0x%llx, i=%zd/%zd\n",
