					...
					bool is_overflow)
{
	...
	if (is_overflow) {
		/* prepare log data for overflow entry */
		sprintf(s->line_buffer, "%s\n", "log overflow");
		strcpy(s->header.data, s->line_buffer);
		strcat(s->header.data, "trusty-log");
		s->header.entry_size = sizeof(uint32_t) +
			strlen(s->line_buffer) + strlen("trusty-log") + 1 +
			sizeof(struct log_data_footer);
		s->footer.log_level = 0;
		s->footer.app_id = -1;
		s->footer.timestamp = 0;
		s->footer.app_name_len = strlen("trusty-log") + 1;
		s->footer.log_len = strlen(s->line_buffer);
	}
	app_name = &s->header.data[s->footer.log_len];

	s->footer.log_len = strlen(s->line_buffer) + 1;
	seq_write(seqf, (const void *)&s->footer,
			sizeof(struct log_data_footer));
	...
	while ((put = log->put) != get) {
		...
		if (alloc - get > log->sz) {
			...
			if (seqf == NULL)
				...
				trusty_debugfs_write(s, seqf, true);
		...
		if (seqf) {
			...
			trusty_debugfs_write(s, seqf, false);
		} else {
