...
static u32 log_size_param = TRUSTY_LOG_DEFAULT_SIZE;

static u32 round_down_to_power_of_two(u32 val)
{
	val |= val >> 1;
	val |= val >> 2;
	val |= val >> 4;
...
static int trusty_log_size_set(const char *val, const struct kernel_param *kp)
{
	...
	requested = round_down_to_power_of_two(requested);
...
{
	sprintf(buffer, "%u", log_size_param);
...
static int trusty_log_init(struct platform_device *pdev)
{
	...
	s->log_num_pages = (log_size_param + sizeof(struct log_rb) +
...
static int trusty_log_probe(struct platform_device *pdev)
{
	...
	if (trusty_log_init(pdev) && log_size_param > TRUSTY_LOG_MIN_SIZE) {
		/* initialization failed. Try again with a 1-page log.  */
