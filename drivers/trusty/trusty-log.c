...
static int log_read_line(struct trusty_log_state *s, int put, int get)
{
	...
	char c = '\0';
        ...
        while (i < sizeof(s->line_buffer) - 1 &&
               get < put) {
            uint32_t offset = get & (s->log->sz/sizeof(struct log_metadata));
            volatile struct log_metadata metadata = s->log->data[offset];
            get++;
...
static void trusty_dump_logs(struct trusty_log_state *s)
{
	...
	while ((put = log->put) != get) {
		...
		rmb();
                ...
                offset = get & (s->log->sz/sizeof(struct log_metadata));
                volatile struct log_metadata metadata = s->log->data[offset];
