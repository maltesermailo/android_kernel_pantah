...
static void tipc_shared_handle_register_locked(struct tipc_shared_handle
					       *new_handle)
{
					...
					**shared_handle)
{
	...
	tipc_shared_handle_register_locked(*shared_handle);
	*shared_handle = NULL;
	mutex_unlock(&shared_handles_lock);
}

static struct tipc_shared_handle *
tipc_shared_handle_take_locked(trusty_shared_mem_id_t obj_id)
{
...
static struct tipc_shared_handle *tipc_shared_handle_take(trusty_shared_mem_id_t
							  obj_id)
{
	struct tipc_shared_handle *out;

	mutex_lock(&shared_handles_lock);
	out = tipc_shared_handle_take_locked(obj_id);
	mutex_unlock(&shared_handles_lock);
	return out;
}
			    ...
			    struct tipc_release_body *req, size_t len)
{
	...
	handle = tipc_shared_handle_take(req->id);
	if (!handle) {
