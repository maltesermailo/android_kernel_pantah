...
enum tipc_fc_event {
	TIPC_FC_CHAN_MARKED_EMPTY_UPON_RX_PUT,
	TIPC_FC_CHAN_TX_PAUSED_UPON_PENDING_MSG_CNT,
	TIPC_FC_CHAN_TX_RESUME_UPON_TRUSTY_MSG,
};
...
void tipc_chan_put_rxbuf(struct tipc_chan *chan, struct tipc_msg_buf *mb)
{
	...
	if ((chan->status_flags & TIPC_CHAN_FLAGS_FC_ACTIVE) == 0 &&
		...
		trace_trusty_ipc_fc(chan, TIPC_FC_CHAN_MARKED_EMPTY_UPON_RX_PUT);
	}
}
...
int tipc_chan_queue_msg(struct tipc_chan *chan, struct tipc_msg_buf *mb)
{
	...
	case TIPC_CONNECTED:
		...
		if (chan->vds->fc_version > 0 &&
			...
			trace_trusty_ipc_fc(chan, TIPC_FC_CHAN_TX_PAUSED_UPON_PENDING_MSG_CNT);
	...
	case TIPC_CTRL_MSGTYPE_TX_EMPTY:
		...
		if (msg->type == TIPC_CTRL_MSGTYPE_TX_FULL) {
			...
			if (chan->state == TIPC_CONNECTED_TX_FULL) {
				...
				trace_trusty_ipc_fc(chan, TIPC_FC_CHAN_TX_RESUME_UPON_TRUSTY_MSG);
