...
static void trusty_heap_dma_buf_release(struct dma_buf *dmabuf)
{
	...
	ret = trusty_reclaim_memory(trusty_dev, buffer->ffa_handle,
				    buffer->sg_table.sgl,
...
int trusty_dma_buf_get_ffa_handle(struct dma_buf *dma_buf, u64 *ffa_handle)
{
	...
	if (dma_buf->ops != &trusty_heap_buf_ops)
		*ffa_handle = TRUSTY_INVALID_FFA_HANDLE;

	*ffa_handle = buffer->ffa_handle;
	return 0;
}
EXPORT_SYMBOL_GPL(trusty_dma_buf_get_ffa_handle);
