				  ...
				  bool ctx)
{
	...
	ret = trusty_share_memory(tvdev->tctx->dev->parent, &tvr->shared_mem_id,
				  &pa, tvr->size, 1, TRUSTY_SHARE_MEMORY_ADD |
				  TRUSTY_SHARE_MEMORY_WRITE);
	if (ret) {
...
static int trusty_virtio_add_devices(struct trusty_ctx *tctx)
{
	...
	ret = trusty_share_memory(tctx->dev->parent, &descr_id, &descr_pa,
				  descr_buf_sz, 1, TRUSTY_SHARE_MEMORY_ADD |
				  TRUSTY_SHARE_MEMORY_WRITE);
	if (ret) {
...
err_load_descr:
	ret_tmp = trusty_share_memory(tctx->dev->parent, &descr_id, &descr_pa,
				      descr_buf_sz, 1,
				      TRUSTY_SHARE_MEMORY_REMOVE);
	if (ret_tmp) {
...
static struct dma_map_ops trusty_virtio_dma_map_ops = {
...
static int trusty_virtio_remove(struct platform_device *pdev)
{
	...
	ret = trusty_share_memory(tctx->dev->parent, &tctx->shared_id,
				  &descr_pa, tctx->shared_sz,
