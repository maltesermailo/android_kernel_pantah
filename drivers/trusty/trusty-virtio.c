...
static void _del_vqs(struct virtio_device *vdev)
{
	...
	for (i = 0; i < tvdev->vring_num; i++, tvr++) {
		...
		/* delete vring */
		if (tvr->vaddr) {
			int ret = trusty_revoke_memory(tvdev->tctx->dev->parent,
						       tvr->shared_mem_id,
						       &tvr->sg, 1);
			if (ret) {
				dev_err(&vdev->dev,
...
err_new_virtqueue:
	ret = trusty_revoke_memory(tvdev->tctx->dev->parent, tvr->shared_mem_id,
				   &tvr->sg, 1);
	if (ret) {
...
err_load_descr:
	ret_tmp = trusty_revoke_memory(tctx->dev->parent, descr_id,
				       &tctx->shared_sg, 1);
	if (ret_tmp) {
...
static int trusty_virtio_remove(struct platform_device *pdev)
{
	...
	/* free shared area */
	ret = trusty_revoke_memory(tctx->dev->parent, tctx->shared_id,
				   &tctx->shared_sg, 1);
	if (ret) {
		dev_err(tctx->dev, "trusty_revoke_memory failed: %d 0x%llx\n",
			ret, tctx->shared_id);
