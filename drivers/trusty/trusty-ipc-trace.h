...
TRACE_EVENT(trusty_ipc_write,
	TP_PROTO(struct tipc_chan *chan, size_t shm_cnt, int len_or_err),
	TP_ARGS(chan, shm_cnt, len_or_err),
	TP_STRUCT__entry(
		__field(u32, chan)
		__array(char, srv_name, MAX_SRV_NAME_LEN)
	...
	TP_printk("chan=%u srv_name=%s shm_cnt=%zu len_or_err=%d", __entry->chan, __entry->srv_name, __entry->shm_cnt, __entry->len_or_err)
);
