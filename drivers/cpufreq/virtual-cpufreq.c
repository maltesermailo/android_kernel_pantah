...
static void virt_scale_freq_tick(void)
{
	...
	cur_freq = (u64)readl_relaxed(base + policy->cpu * PER_CPU_OFFSET
			+ REG_CUR_FREQ_KHZ_OFFSET);

	cur_freq <<= SCHED_CAPACITY_SHIFT;
	scale = div_u64(cur_freq, max_freq);

	if (unlikely(scale > SCHED_CAPACITY_SCALE))
...
{
	writel_relaxed(policy->cached_target_freq, base + policy->cpu * PER_CPU_OFFSET
			+ REG_SET_FREQ_KHZ_OFFSET);
	return 0;
}
...
static int virt_cpufreq_cpu_init(struct cpufreq_policy *policy)
{
	 ...
	 * abort is the same thread being set with performance constraints(Ex.
	 * UClamp settings). This simplifies the VMM(Virtual Machine Manager)
	 * having to find the correct vCPU thread and/or facing permission
	 * issues when configuring other threads.
	 */
	 ...
	 * from the vCPU thread due to frequency update latencies or higher
	 * frequency requirements from other threads. This additional FIE
