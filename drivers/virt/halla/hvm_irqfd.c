 ...
 */
static void irqfd_set_irq(struct hvm *hvm, uint32_t irq, int level)
{
	if (level)
...
/**
 * irqfd_is_active() - Assumes hvm->irqfds.lock is held.
 ...
 *			assumes hvm->irqfds.lock is held.
 * @irqfd: Pointer to hvm_kernel_irqfd.
 */
 ...
 * * 0			- Success
 */
static int irqfd_wakeup(wait_queue_entry_t *wait, uint32_t mode, int sync,
			void *key)
{
	struct hvm_kernel_irqfd *irqfd =
