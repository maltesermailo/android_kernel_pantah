...
__hvm_reclaim_guest_memory(uint16_t vmid, uint64_t gfn, unsigned long nr_pages)
{
	...
	pr_debug("Relcaim gfn[%#llx]/pages[%ld]\n", gfn, nr_pages);
	...
	while (left_pages > 0) {
		...
		ret = exynos_hvc(HVC_FID_HVM_RECLAIM_GUEST_MEMORY,
				 vmid, ipa, order, 0);
		if (ret) {
			pr_err("Failed to reclaim guest memory (%#lx)\n", ret);
			pr_err("- ipa[%#llx]/order[%#lx]\n", ipa, order);
			break;
		}
...
static void hvm_memslot_reclaim_guest_memory(struct hvm *hvm)
{
	...
	for (unsigned int i = 0; i < HVM_MAX_MEM_REGION; i++) {
		...
		pr_debug("VM%d memslot[%d] to be reclaimed\n",
				hvm->vm_id, i);

		if (__hvm_reclaim_guest_memory(hvm->vm_id,
					       slot->base_gfn,
					       slot->npages))
			break;
	}
...
static void hvm_reclaim_guest_memory(struct hvm *hvm)
{
	hvm_memslot_reclaim_guest_memory(hvm);
}
...
static void hvm_reclaim_guest_stage2_pgtable(struct hvm *hvm)
{
	...
	ret = exynos_hvc(HVC_FID_HVM_RECLAIM_GUEST_S2_PGTABLE,
			 hvm->vm_id, 0, 0, 0);
	if (ret) {
		pr_err("Failed to reclaim guest stage2 pgtable (%#lx)\n",
				ret);
		return;
	}
...
static void hvm_destroy_vm(struct hvm *hvm)
{
	...
	ret = hvm_pre_destory_vm(hvm);
	if (ret) {
		/* Destroying VM is failed in Halla but keep destroying it */
		pr_err("Failed to pre destroy VM in Halla ret[%lx]\n", ret);
	}

	hvm_vm_irqfd_release(hvm);

	hvm_reclaim_guest_memory(hvm);
	...
	pr_info("Destroyed VM-%u completely\n", hvm->vm_id);
...
int hvm_dev_ioctl_create_vm(unsigned long vm_type)
{
	...
	if (ret) {
		pr_err("Failed to init arch drv\n");
		kfree(hvm);
		return ret;
	}
