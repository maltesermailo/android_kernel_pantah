...
static int pin_one_page(struct hvm *vm, unsigned long hva, uint64_t gpa,
			struct page **out_page)
{
	uint32_t flags = FOLL_HWPOISON | FOLL_LONGTERM | FOLL_WRITE;
...
static int hvm_register_vm_pgtable(struct hvm *hvm)
{
	...
	if (buf == NULL) {
		pr_err("%s: Failed to allocate VM stage 2 table\n",
				__func__);
		return -ENOMEM;
	}

	pr_debug("VM pgtable %#lx[%#lx]\n", (unsigned long)buf, buf_pa);

	ret = exynos_hvc(HVC_FID_HVM_REGISTER_VM_PGTABLE,
			 hvm->vm_id, buf_pa, size, 0);
	if (ret) {
		pr_err("%s: Fail to set VM stage2 table buf ret = %lx\n",
				__func__, ret);
	...
	pr_debug("%s: VM pgtable registration success\n", __func__);
...
int hvm_init_vm_stage2_mmu(struct hvm *hvm)
{
	...
	if (buf == NULL) {
		pr_err("%s: Failed to allocate VM stage 2 table buffer\n",
				__func__);
		return -ENOMEM;
	}

	pr_info("VM base pgtable %#lx[%#lx]\n", (unsigned long)buf, buf_pa);
	...
	pr_info("%s: VM stage2 table buffer set successfully\n",
			__func__);
...
static int hvm_vm_allocate_guest_page(struct hvm *vm, struct hvm_memslot *slot,
				uint64_t gfn, uint64_t *pfn)
{
	struct page *page = NULL;
	unsigned long hva;
	int ret;

	if (hvm_gfn_to_hva_memslot(slot, gfn, (uint64_t *)&hva) != 0)
		return -EINVAL;
...
static int hvm_map_guest(struct hvm *hvm, int memslot_id,
		  uint64_t pfn, uint64_t gfn, uint64_t nr_pages)
{
	unsigned long ret;
	uint64_t ids;
	...
	if (ret) {
		pr_err("VM pgtable alloc failure (%#lx)\n", ret);
		return (int)ret;
	}

	ids = (uint32_t)memslot_id;
...
static int handle_demand_page(struct hvm *vm, int memslot_id, uint64_t gfn)
{
	int ret;
	uint64_t pfn;
 ...
 * * -EFAULT		- Not found
 */
static int hvm_find_memslot(struct hvm *vm, uint64_t gfn)
{
	int i;
