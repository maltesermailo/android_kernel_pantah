...
register_memslot_addr_range(struct gzvm *gzvm, struct gzvm_memslot *memslot)
{
	...
	pr_debug("%s ipa=%llx, ipa_end=%llx, remain_pages=%d\n", __func__,
				...
				struct gzvm_userspace_memory_region *mem)
{
	...
	pr_debug("slot=%u, flags=%x, gpa=%llx, size=%llu, uva=%llx\n",
	...
	if (!vma) {
		pr_err("Error to find vma corresponding userva\n");
		return -EFAULT;
	}
	size = vma->vm_end - vma->vm_start;
	if (size != mem->memory_size) {
		pr_err("Size does not match: vma:%lx mem:%llx\n", size,
		       mem->memory_size);
		return -EINVAL;
	}
			 ...
			 u32 irq, bool level)
{
	...
	pr_debug("%s vcpu=%u irq=%u irq_type=%d level=%d\n", __func__,
...
static int gzvm_vm_ioctl_create_device(struct gzvm *gzvm, void __user *argp)
{
	...
	pr_debug("%s type:%d addr:0x%llx, size=0x%llx", __func__,
