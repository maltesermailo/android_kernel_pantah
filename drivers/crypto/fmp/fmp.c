...
static bool fmp_check_fips(struct bio *bio, struct exynos_fmp *fmp)
{
	...
	if ((bio->bi_io_vec) && !IS_ERR(bio->bi_io_vec)) {
		page = bio->bi_io_vec[0].bv_page;
		if (page && !IS_ERR(page)) {
			if ((page->private) && !IS_ERR((void *)page->private)) {
				bh = (void *)page->private;
				if ((void *)bh->b_private == (void *)fmp) {
					find = true;
	...
	if (find) {
		fmp->fips_run--;
		ci = &fmp->test_data->ci;
		dev_dbg(fmp->dev, "%s: find fips(%d) with algo:%d, enc:%d, key_size:%d\n",
...
#undef FIPS_EXEC_PROBE
#define FIPS_EXEC_WORK
#undef FIPS_EXEC_FIRST_IO

static int fmp_fips_run(struct exynos_fmp *fmp)
{
	int ret = 0;

	if (!atomic_read(&fmp->fips_start)) {
		atomic_set(&fmp->fips_start, 1);
		dev_info(fmp->dev, "%s works\n", __func__);
...
#ifdef FIPS_EXEC_FIRST_IO
	if (!atomic_read(&fmp->fips_start)) {
		dev_info(fmp->dev, "%s: run fips on first io\n", __func__);
		fmp_fips_run(fmp);
	}
#endif
...
#ifdef FIPS_EXEC_WORK
static void fmp_test_work_func(struct work_struct *work)
{
	struct exynos_fmp *fmp = get_fmp();
...
static DECLARE_DEFERRABLE_WORK(fmp_test_work, fmp_test_work_func);
#endif
...
#ifdef FIPS_EXEC_PROBE
	/* init fips */
	ret = fmp_fips_run(fmp);
#endif
#ifdef FIPS_EXEC_WORK
	schedule_delayed_work(&fmp_test_work, msecs_to_jiffies(1000));
#endif
#endif
