			...
			const char __user *buffer, size_t count, loff_t *ppos)
{
	 ...
	 *
	 * Using uid_entry_tmp to get the result of Update_io_stats_uid_locked,
	 * so that we can unlock_uid during update_io_stats_uid_locked, in order
	...
	unlock_uid(uid);
	update_io_stats_uid_locked(&uid_entry_tmp);
