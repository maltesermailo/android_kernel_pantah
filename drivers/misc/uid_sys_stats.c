...
static int calc_uid_cputime(struct uid_entry *uid_entry,
...
static int uid_cputime_show(struct seq_file *m, void *v)
{
	...
	for (bkt = 0, uid_entry = NULL; uid_entry == NULL &&
		...
		hlist_for_each_entry(uid_entry, &hash_table[bkt], hash) {
			...
			ret = calc_uid_cputime(uid_entry, &total_utime, &total_stime);
			if (ret) {
...
static int update_io_stats_uid(struct uid_entry *uid_entry, bool locked_uid)
{
	struct user_namespace *user_ns = current_user_ns();
	struct task_struct *p, *t;
	struct io_stats io;
...
static int uid_io_show(struct seq_file *m, void *v)
{
	...
	for (bkt = 0, uid_entry = NULL; uid_entry == NULL && bkt < HASH_SIZE(hash_table);
		...
		hlist_for_each_entry(uid_entry, &hash_table[bkt], hash) {
			...
			ret = update_io_stats_uid(uid_entry, true);
			if (ret) {
				unlock_uid_by_bkt(bkt);
				return ret;
			}
			...
			const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	unlock_uid(uid);
	update_io_stats_uid(&uid_entry_tmp, false);
