				   ...
				   struct zram_pp_ctl *ctl)
{
	...
	if (cmd == ZRAM_ANDROID_IOC_PROCESS_RANGE_WRITEBACK) {
		struct zram_android_ioc_data_process_range_writeback *prwb;

		prwb = &ioc_data->data.process_range_writeback;
		pidfd = prwb->pidfd;
		...
		struct zram_android_ioc_data_process_writeback *pwb;

		pwb = &ioc_data->data.process_writeback;
		pidfd = pwb->pidfd;
		start_addr = 0;
		nr_remaining_pages = 0;
	} else {
		pidfd = ioc_data->data.process_prefetch.pidfd;
	}
...
static int zram_ioctl_process_writeback(struct zram *zram, unsigned int cmd,
					struct zram_android_ioc_data *ioc_data)
{
	...
	ret = zram_ioctl_process_scan(zram, cmd, ioc_data, pp_ctl);
	if (!ret)
	...
	if (cmd == ZRAM_ANDROID_IOC_PROCESS_RANGE_WRITEBACK) {
		...
		pwb = &ioc_data->data.process_writeback;
		pwb->written_bytes = wb_ctl->processed_bytes;
	}
	       ...
	       unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == ZRAM_ANDROID_IOC_PROCESS_WRITEBACK ||
	    cmd == ZRAM_ANDROID_IOC_PROCESS_RANGE_WRITEBACK) {
		ret = zram_ioctl_process_writeback(zram, cmd, &ioc_data);

		if (copy_to_user(argp, &ioc_data, sizeof(ioc_data)))
			ret = -EFAULT;
	} else if (cmd == ZRAM_ANDROID_IOC_PROCESS_PREFETCH) {
		ret = zram_ioctl_process_prefetch(zram, &ioc_data);
	}
