...
int binder_try_unload_builtin(bool is_rust)
{
	...
	if (binder_loaded)
		ret = -EBUSY;
	else if (binder_use_rust != is_rust)
		ret = -EPERM;
	else
		binder_loaded = true;
...
int on_binderfs_mount(bool is_rust)
{
	...
	if (binder_loaded && binder_use_rust != is_rust) {
		ret = -EINVAL;
		goto out;
	}
	 ...
	 * requested.
	 */
	if (binder_use_rust && !is_rust)
		pr_warn("Using C Binder even though binder.impl=rust is set.\n");
	if (!binder_use_rust && is_rust)
		pr_warn("Using Rust Binder even though binder.impl=c is set.\n");

	binder_use_rust = is_rust;
	binder_loaded = true;
