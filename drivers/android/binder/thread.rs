    ...
    fn write(self: &Arc<Self>, req: &mut BinderWriteRead) -> Result {
        ...
        while reader.len() >= size_of::<u32>() && self.inner.lock().return_work.is_unused() {
            ...
            match cmd {
                ...
                BC_REQUEST_FREEZE_NOTIFICATION => self.process.request_freeze(&mut reader)?,
                BC_CLEAR_FREEZE_NOTIFICATION => self.process.clear_freeze(&mut reader)?,
                BC_FREEZE_NOTIFICATION_DONE => self.process.freeze_done(&mut reader)?,
