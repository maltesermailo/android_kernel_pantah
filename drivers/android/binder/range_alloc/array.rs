...
impl<T> ArrayRangeAllocator<T> {
    ...
    pub(crate) fn find_empty_range(&self, size: usize) -> Option<(usize, usize)> {
    ...
    pub(crate) fn reserve_new(&mut self, size: usize, is_oneway: bool, pid: Pid) -> Result<usize> {
        ...
        let (insert_at_idx, insert_at_offset) = self.find_empty_range(size).ok_or(ENOSPC)?;
