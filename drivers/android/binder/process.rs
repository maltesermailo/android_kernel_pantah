...
impl ProcessInner {
    ...
    fn get_existing_node_ref(
        ...
        Ok(self
            ...
            .map(|node| self.new_node_ref(node, strong, thread, process)))
    }
...
impl Process {
    ...
    fn get_current_thread(self: ArcBorrow<'_, Self>) -> Result<Arc<Thread>> {
        ...
        self.threads.insert(id as u64, ta.clone())?;
        Ok(ta)
    }
    ...
    pub(crate) fn get_node(
        ...
        {
            let mut inner = self.inner.lock();
            if let Some(node) = inner.get_existing_node_ref(ptr, cookie, strong, thread, &self)? {
                return Ok(node);
            }
        }
