...
pub(crate) fn trace_ioctl_done(ret: Result) {
    // SAFETY: Always safe to call.
    unsafe { rust_binder_ioctl_done(ret.map(|err| err.to_errno()).unwrap_or(0)) }
}

#[inline]
pub(crate) fn trace_read_done(ret: Result) {
    // SAFETY: Always safe to call.
    unsafe { rust_binder_read_done(ret.map(|err| err.to_errno()).unwrap_or(0)) }
}

#[inline]
pub(crate) fn trace_write_done(ret: Result) {
    // SAFETY: Always safe to call.
    unsafe { rust_binder_write_done(ret.map(|err| err.to_errno()).unwrap_or(0)) }
}
