...
impl Thread {
    pub(crate) fn new(id: i32, process: Arc<Process>) -> Result<Arc<Self>> {
        ...
        Ok(Arc::pin_init(pin_init!(Thread {
            id,
            process,
    ...
    fn get_work_local(self: &Arc<Self>, mut wait: bool) -> Result<Option<Arc<dyn DeliverToRead>>> {
        if self.inner.lock().looper_need_return {
            wait = false;
        }
    ...
    fn get_work(self: &Arc<Self>, mut wait: bool) -> Result<Option<Arc<dyn DeliverToRead>>> {
        ...
        loop {
            ...
            if signal_pending || inner.looper_need_return {
                ...
                let res = match self.inner.lock().pop_work() {
                    ...
                    None if signal_pending => Err(ERESTARTSYS),
                    None => Ok(None),
                };
                return res;
            }
        ...
        match object {
            ...
            BinderObjectRef::Fd(obj) => {
                ...
                const FD_FIELD_OFFSET: usize =
                    kernel::offset_of!(bindings::binder_fd_object, __bindgen_anon_1) as usize;
    ...
    pub(crate) fn copy_transaction_data<'a>(
        ...
        if offsets_size > 0 {
                ...
                .enumerate()
            {
                ...
                view.alloc.set_info_offsets(offsets_start..index_offset);
            }
        }
