				       ...
				       loff_t buf_offs, loff_t len, struct dmabuf_load_param *param)
{
	...
	if (direct_read_len) {
		param->iov[nr_segs].iov_base = (u8 *)map.vaddr + direct_read_start_offs;
...
static void dmabuf_content_load_complete(struct dmabuf_load_param *param)
{
	...
	if (param->direct_read_len &&
	    ((u8 *)param->map.vaddr + param->direct_read_start_offs != dst)) {
		src = (u8 *)param->map.vaddr + param->direct_read_start_offs;
			       ...
			       loff_t file_offs, loff_t buf_offs, loff_t len)
{
	...
	while (file_read_len > 0) {
		...
		WARN_ON(!PAGE_ALIGNED(param.iter.count));
