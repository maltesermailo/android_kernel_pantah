...
static int folios_content_get_mappable(struct wrap_content *content,
				       struct device *dev,
				       struct mappable_data *mappable)
{
	struct wrap_content_folios *folios_content;
	...
	folios_content->table = table;
out:
	mappable->type = MAPPABLE_FOLIO_TABLE;
	mappable->table = folios_content->table;

	return 0;
}

static void folios_content_put_mappable(struct wrap_content *content)
{
	struct wrap_content_folios *folios_content;
	...
	free_folio_table(folios_content->table);
	kfree(folios_content->table);
	folios_content->table = NULL;
}
...
static struct wrap_content_operations folios_content_ops = {
	...
	.get_mappable		= folios_content_get_mappable,
	.put_mappable		= folios_content_put_mappable,
};
...
static int rd_file_content_get_mappable(struct wrap_content *content,
					struct device *dev,
					struct mappable_data *mappable)
{
	struct wrap_content_rd_file *rd_file_content;
	...
	rd_file_content->table = table;
out:
	mappable->type = MAPPABLE_FOLIO_TABLE;
	mappable->table = rd_file_content->table;

	return 0;
}

static void rd_file_content_put_mappable(struct wrap_content *content)
{
	struct wrap_content_rd_file *rd_file_content;
	...
	free_folio_table(rd_file_content->table);
	kfree(rd_file_content->table);
	rd_file_content->table = NULL;
}
...
static struct wrap_content_operations rd_file_content_ops = {
	...
	.get_mappable		= rd_file_content_get_mappable,
	.put_mappable		= rd_file_content_put_mappable,
};
...
static int dmabuf_content_get_mappable(struct wrap_content *content,
				       struct device *dev,
				       struct mappable_data *mappable)
{
	struct wrap_content_dmabuf *dmabuf_content;
...
out:
	mappable->type = MAPPABLE_DMABUF_ATTACHMENT;
	mappable->attachment = dmabuf_content->attachment;

	return 0;
}

static void dmabuf_content_put_mappable(struct wrap_content *content)
{
	struct wrap_content_dmabuf *dmabuf_content;
...
static struct wrap_content_operations dmabuf_content_ops = {
	...
	.put_mappable		= dmabuf_content_put_mappable,
};
...
unlock:
	...
	ret = ctx->content->ops->get_mappable(ctx->content, dev, mappable);
	if (ret) {
		spin_lock(&ctx->lock);
		ctx->owner.dev = NULL;
		spin_unlock(&ctx->lock);
...
unlock:
	...
	if (!ret)
		ctx->content->ops->put_mappable(ctx->content);
...
static int wrap_file(struct wrap_ctx *ctx, unsigned long arg)
{
	...
	dmabuf = dma_buf_get(wrapfd_wrap.fd);
	if (!IS_ERR(dmabuf)) {
		bool writable = !!(wrapfd_wrap.prot & PROT_WRITE);
