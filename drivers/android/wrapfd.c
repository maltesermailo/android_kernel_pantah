...
static void prepare_iocb_request(struct dmabuf_load_param *param, struct file *file)
{
	...
	if (param->start_bounce_len) {
		param->iov[nr_segs].iov_base = param->bounce_bufs[cur_bounce_page];
				       ...
				       loff_t buf_offs, loff_t len, struct dmabuf_load_param *param)
{
	...
	param->dmabuf = dmabuf;
	memcpy(param->bounce_bufs, bounce_bufs, sizeof(param->bounce_bufs));
...
static void dmabuf_content_load_complete(struct dmabuf_load_param *param)
{
	...
	if (param->start_bounce_len) {
		src = param->bounce_bufs[cur_bounce_page] + offset_in_page(param->file_offs);
	...
	for (i = 0; i < param->end_bounce_len / PAGE_SIZE; i++) {
		src = param->bounce_bufs[cur_bounce_page];
...
out:
	...
	for (i = 0; i < ((param->start_bounce_len + param->end_bounce_len) / PAGE_SIZE); i++)
		free_page((unsigned long)param->bounce_bufs[i]);
}
