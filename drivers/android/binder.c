...
#define binder_reset_error_info(einfo, _id) \
	do { \
		(einfo)->ee.id = _id; \
		(einfo)->ee.command = BR_OK; \
		(einfo)->ee.param = 0; \
		(einfo)->strerr[0] = '\0'; \
	} while (0)

#define binder_set_error_info(einfo, _command, _param, format...) \
	do { \
		if ((_command) != BR_OK && (einfo)->ee.command != BR_OK) \
			break; \
		(einfo)->ee.command = _command; \
		(einfo)->ee.param = _param; \
		snprintf((einfo)->strerr, sizeof((einfo)->strerr), format); \
	} while (0)

#define binder_user_error_info(einfo, command, param, format...) \
	do { \
		binder_set_error_info(einfo, command, param, format); \
		binder_user_error("%s\n", (einfo)->strerr); \
	} while (0)
...
static void binder_set_from_extended_error(struct binder_transaction *t,
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	binder_reset_error_info(&einfo, t_debug_id);
	binder_inner_proc_lock(proc);
	thread->ee = einfo.ee;
	binder_inner_proc_unlock(proc);
	...
	if (reply) {
		...
		if (in_reply_to == NULL) {
			binder_inner_proc_unlock(proc);
			binder_user_error_info(&einfo, BR_FAILED_REPLY, -EPROTO,
		...
		if (target_thread == NULL) {
			/* annotation for sparse */
			__release(&target_thread->proc->inner_lock);
			binder_set_error_info(&einfo, BR_DEAD_REPLY, 0,
		...
		if (tr->target.handle) {
			...
			if (ref)
				...
				binder_user_error_info(&einfo, BR_FAILED_REPLY,
					-EINVAL,
		...
		if (!target_node) {
			 ...
			 */
			binder_set_error_info(&einfo, return_error, -EINVAL,
				"cannot find target node");
			goto err_dead_binder;
		}
		e->to_node = target_node->debug_id;
		if (WARN_ON(proc == target_proc)) {
			binder_set_error_info(&einfo, BR_FAILED_REPLY, -EINVAL,
				"self transactions not allowed");
			goto err_invalid_target_handle;
		}
		if (security_binder_transaction(proc->cred,
						target_proc->cred) < 0) {
			binder_set_error_info(&einfo, BR_FAILED_REPLY, -EPERM,
	...
	if (t == NULL) {
		binder_set_error_info(&einfo, BR_FAILED_REPLY, -ENOMEM,
	...
	if (target_node && target_node->txn_security_ctx) {
		...
		if (ret) {
			binder_set_error_info(&einfo, BR_FAILED_REPLY, ret,
	...
	if (binder_alloc_copy_user_to_buffer(
				...
				tr->offsets_size)) {
		binder_user_error_info(&einfo, BR_FAILED_REPLY, -EFAULT,
			"invalid offsets ptr");
		goto err_copy_data_failed;
	}
	if (!IS_ALIGNED(tr->offsets_size, sizeof(binder_size_t))) {
		binder_user_error_info(&einfo, BR_FAILED_REPLY, -EINVAL,
		...
		case BINDER_TYPE_WEAK_BINDER: {
			...
			if (ret < 0 ||
			    binder_alloc_copy_to_buffer(&target_proc->alloc,
							...
							object_offset,
							fp, sizeof(*fp))) {
				binder_set_error_info(&einfo, BR_FAILED_REPLY,
	...
	ret = binder_do_deferred_txn_copies(&target_proc->alloc, t->buffer,
					    &sgc_head, &pf_head);
	if (ret) {
		binder_user_error_info(&einfo, BR_FAILED_REPLY, ret,
...
err_dead_proc_or_thread:
	binder_set_error_info(&einfo, return_error, -ESRCH,
...
err_invalid_target_handle:
	...
	binder_debug(BINDER_DEBUG_FAILED_TRANSACTION,
		     "%d:%d transaction to %d:%d failed, size %lld/%lld, error %d/%d/%d: %s\n",
		     proc->pid, thread->pid,
		     target_proc ? target_proc->pid : 0,
		     target_thread ? target_thread->pid : 0,
		     (u64)tr->data_size, (u64)tr->offsets_size,
		     einfo.ee.id, einfo.ee.command, einfo.ee.param,
		     einfo.strerr);
	...
	if (in_reply_to) {
		trace_android_vh_binder_restore_priority(in_reply_to, current);
		binder_restore_priority(current, in_reply_to->saved_priority);
		binder_set_from_extended_error(in_reply_to, &einfo.ee);
		thread->return_error.cmd = BR_TRANSACTION_COMPLETE;
		binder_enqueue_thread_work(thread, &thread->return_error.work);
		binder_send_failed_reply(in_reply_to, einfo.ee.command);
	} else {
		binder_inner_proc_lock(proc);
		thread->ee = einfo.ee;
		binder_inner_proc_unlock(proc);
		thread->return_error.cmd = einfo.ee.command;
		binder_enqueue_thread_work(thread, &thread->return_error.work);
	}
}
					...
					struct binder_transaction_log_entry *e)
{
	...
	smp_rmb();
	seq_printf(m,
		   "%d: %s from %d:%d to %d:%d context %s node %d handle %d size %d:%d err %d/%d: %s",
		   e->debug_id, (e->call_type == 2) ? "reply" :
		   ((e->call_type == 1) ? "async" : "call "), e->from_proc,
		   e->from_thread, e->to_proc, e->to_thread, e->context_name,
		   e->to_node, e->target_handle, e->data_size, e->offsets_size,
		   e->einfo.ee.command, e->einfo.ee.param, e->einfo.strerr);
	/*
