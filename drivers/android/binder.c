...
{
	binder_size_t __user *u_offp, *u_off_start, *u_off_end;
	...
	u_off_start = (binder_size_t __user *)
		(buffer->user_data + off_start_offset);
	if (failed_at)
		u_off_end = failed_at;
	else
		u_off_end = (void __user *)u_off_start + buffer->offsets_size;
	for (u_offp = u_off_start; u_offp < u_off_end; u_offp++) {
		struct binder_object_header *hdr;
		size_t object_size;
		struct binder_object object;
		binder_size_t object_offset;
		binder_size_t buffer_offset = (uintptr_t)u_offp -
