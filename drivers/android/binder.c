			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (reply) {
		...
		if (target_thread == NULL) {
			...
			__release(&target_thread->proc->inner_lock);
			binder_set_extended_error(ee, BINDER_EE_PARAM_ERRNO,
				BR_DEAD_REPLY, 0, "reply target not found");
	...
	if (IS_ERR(t->buffer)) {
		...
		case -ESRCH:
			binder_set_extended_error(ee, BINDER_EE_PARAM_ERRNO,
				BR_DEAD_REPLY, ret, "cannot allocate buffer:"
				" vma cleared, target dead or dying");
			break;
		case -ENOSPC:
			binder_set_extended_error(ee, BINDER_EE_PARAM_ERRNO,
				BR_FAILED_REPLY, ret, "cannot allocate buffer:"
				" no space left");
			break;
		case -ENOMEM:
			binder_set_extended_error(ee, BINDER_EE_PARAM_ERRNO,
	...
	if (binder_alloc_copy_user_to_buffer(
				...
				tr->offsets_size)) {
		binder_set_extended_error(ee, BINDER_EE_PARAM_ERRNO,
			BR_FAILED_REPLY, -EFAULT, "invalid offsets ptr");
		goto err_copy_data_failed;
	}
	if (!IS_ALIGNED(tr->offsets_size, sizeof(binder_size_t))) {
		binder_set_extended_error(ee, BINDER_EE_PARAM_ERRNO,
			BR_FAILED_REPLY, -EINVAL, "invalid offsets size %lld",
			(u64)tr->offsets_size);
		goto err_bad_offset;
	}
	...
	if (binder_alloc_copy_user_to_buffer(
		...
		binder_set_extended_error(ee, BINDER_EE_PARAM_ERRNO,
			BR_FAILED_REPLY, -EFAULT, "invalid data ptr");
		goto err_copy_data_failed;
	}
...
err_invalid_target_handle:
	...
	binder_debug(BINDER_DEBUG_FAILED_TRANSACTION,
		     "%d:%d transaction with size %lld-%lld failed %d/%d/%d: %s\n",
		     proc->pid, thread->pid,
		     (u64)tr->data_size, (u64)tr->offsets_size,
		     ee->code, ee->command, ee->param, ee->string);
					...
					struct binder_transaction_log_entry *e)
{
	...
	smp_rmb();
	seq_printf(m,
		   "%d: %s from %d:%d to %d:%d context %s node %d handle %d size %d:%d error %d:%d:%d: %s",
		   e->debug_id, (e->call_type == 2) ? "reply" :
		   ((e->call_type == 1) ? "async" : "call "), e->from_proc,
		   e->from_thread, e->to_proc, e->to_thread, e->context_name,
		   e->to_node, e->target_handle, e->data_size, e->offsets_size,
		   e->extended_error.code, e->extended_error.command,
		   e->extended_error.param, e->extended_error.string);
	/*
