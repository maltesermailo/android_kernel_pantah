 ...
 *
 * Copy the binder object at the given offset into @offset. If @u is
...
/**
 * struct binder_ptr_fixup - data to be fixed-up in target bufer
 ...
 * @ret:	default return value. If non-zero, copies are inhibited
 *              but lists are cleaned up.
 *
 ...
 *
 * If there is a transaction error, this function should be called
 * with a non-zero @ret to consume and free the lists without actually
 * copying any data.
 *
 ...
 *
 * We are guarenteed that repeated calls to this function will have
				     ...
				     struct binder_transaction *in_reply_to)
{
	...
	binder_size_t fda_offset, ufda_offset;
	struct binder_proc *proc = thread->proc;
	int ret;
	...
	fda_offset = (parent->buffer - (uintptr_t)t->buffer->user_data) +
		fda->parent_offset;
	ufda_offset = (uparent->buffer - (uintptr_t)u) + fda->parent_offset;
	...
	for (fdi = 0; fdi < fda->num_fds; fdi++) {
		...
		binder_size_t uoffset = ufda_offset + fdi * sizeof(fd);

		ret = copy_from_user(&fd, u + uoffset, sizeof(fd));
		if (!ret)
	...
	for (buffer_offset = off_start_offset; buffer_offset < off_end_offset;
		...
		case BINDER_TYPE_FDA: {
			...
			if (user_parent_size != sizeof(user_object.bbo)) {
				binder_user_error("%d:%d invalid data ptr object size: %lld vs %lld\n",
						  proc->pid, thread->pid,
						  user_parent_size,
			...
			/*
			 * Set user_offset to copy the FDA object from
			 * the source to target buffer
			 */
...
err_copy_data_failed:
	/*
	 * by passing -EINVAL, copies are inhibited, but lists are
	 * cleaned up.
	 */
	binder_do_deferred_txn_copies(&target_proc->alloc, t->buffer,
				      &sgc_head, &pf_head, -EINVAL);
	binder_free_txn_fixups(t);
