				    ...
				    struct binder_thread *thread)
{
	...
	trace_android_vh_binder_proc_transaction(current, proc->tsk,
		...
		pending_async);
	trace_android_vh_binder_special_task(t, proc, thread, NULL, !oneway, &special_task);
	...
	if (thread) {
		...
		if (special_task)
			trace_android_vh_binder_enqueue_work_ilocked(&t->work, &proc->todo);
		else
		...
		if (special_task)
			trace_android_vh_binder_enqueue_work_ilocked(&t->work, &node->async_todo);
		else
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	trace_android_vh_binder_transaction_buffer(tr, t, target_proc);
		...
		struct binder_buffer *buffer, bool is_failure)
{
	...
	if (buffer->async_transaction && buffer->target_node) {
		...
		if (!w) {
			...
			trace_android_vh_binder_special_task(NULL, proc, thread,
					w, false, &special_task);
			if (special_task)
				trace_android_vh_binder_enqueue_work_ilocked(w, &proc->todo);
			else
