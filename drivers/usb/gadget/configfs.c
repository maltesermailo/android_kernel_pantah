...
static void android_work(struct work_struct *data)
{
	...
	if (status[0]) {
		...
		dev_info(&cdev->gadget->dev, "%s: sent uevent %s\n", __func__, connected[0]);
		uevent_sent = true;
	}
	...
	if (!uevent_sent) {
		dev_err(&cdev->gadget->dev, "%s: did not send uevent (%d %d %p)\n", __func__,
...
static struct attribute *android_usb_class_attrs[] = {
	&dev_attr_state.attr,
	NULL,
};

static const struct attribute_group android_usb_group = {
	.attrs = android_usb_class_attrs,
};

static const struct attribute_group *android_usb_groups[] = {
	&android_usb_group,
	NULL,
};
...
struct device *create_function_device(char *name)
{
	if (android_device && !IS_ERR(android_device))
		return device_create(&android_usb_class, android_device,
...
static int android_device_create(struct gadget_info *gi)
{
	...
	gi->dev = device_create(&android_usb_class, NULL,
...
#ifdef CONFIG_USB_CONFIGFS_UEVENT
	ret = class_register(&android_usb_class);
	if (ret)
