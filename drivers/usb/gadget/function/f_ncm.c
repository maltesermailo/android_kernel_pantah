...
static int ncm_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	ncm->netdev = gether_setup_default();
	if (IS_ERR(ncm->netdev)) {
	...
	scoped_guard(mutex, &ncm_opts->lock)
		gether_apply_opts(ncm->netdev, &ncm_opts->net_opts);

	gether_set_gadget(ncm->netdev, cdev->gadget);
	status = gether_register_netdev(ncm->netdev);
	if (status) {
		free_netdev(ncm->netdev);
		ncm->netdev = NULL;
		kfree(f->os_desc_table);
		f->os_desc_n = 0;
		return status;
	...
	status = gether_get_host_addr_cdc(ncm->netdev, ncm->ethaddr,
					  sizeof(ncm->ethaddr));
...
static struct usb_function_instance *ncm_alloc_inst(void)
{
	...
	struct f_ncm_opts *opts = kzalloc(sizeof(*opts), GFP_KERNEL);
	if (!opts)
