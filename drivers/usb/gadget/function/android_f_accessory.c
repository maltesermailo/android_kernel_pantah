...
static void acc_complete_in(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (req->status == -ESHUTDOWN) {
		pr_debug("%s set disconnected", __func__);
	...
	size_t count, loff_t *pos)
{
	...
	if (dev->disconnected) {
		pr_debug("%s, disconnected", __func__);
		return -ENODEV;
	}
	...
	if (!dev->rx_req[0]) {
		pr_warn("%s: USB request already handled/freed", __func__);
		r = -EINVAL;
		goto done;
	}
...
requeue_req:
	...
	if (ret < 0) {
		...
		if (ret != 0) {
			...
			pr_debug("%s: cancelling failed %d", __func__, ret);
		}
		goto done;
	}
...
EXPORT_SYMBOL_GPL(acc_req_match_composite);
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	if (bRequestType == (USB_DIR_OUT | USB_TYPE_VENDOR)) {
		if (bRequest == ACCESSORY_START) {
			...
			spin_lock_irqsave(&dev->lock, flags);
			hid = acc_hid_get(&dev->new_hid_list, wValue);
			spin_unlock_irqrestore(&dev->lock, flags);
			if (!hid) {
				value = -EINVAL;
				goto err;
			...
			spin_lock_irqsave(&dev->lock, flags);
			hid = acc_hid_get(&dev->hid_list, wValue);
			spin_unlock_irqrestore(&dev->lock, flags);
			if (!hid) {
				value = -EINVAL;
				goto err;
...
EXPORT_SYMBOL_GPL(acc_setup_composite);
