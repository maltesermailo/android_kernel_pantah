...
static void put_acc_dev(struct acc_dev *dev)
{
	...
	/*
	 * This is not best engineering practice, and does cause coupling with kref internal structure.
	 * It might cause an issue if kref internal refcount structure is changed. We will remove this
	 * implementation in the next kernel version.
	 */
	if (refcount_dec_and_lock_irqsave(&acc_dev_instance->kref.refcount, &acc_dev_instance_lock,
		&flags))
