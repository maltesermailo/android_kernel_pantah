...
static int cros_ucsi_read_message_in(struct ucsi *ucsi, void *val, size_t val_len)
{
...
{
	return cros_ucsi_sync_write(ucsi, UCSI_CONTROL, &command, sizeof(command));
}

static int cros_ucsi_async_control(struct ucsi *ucsi, u64 command)
{
	return cros_ucsi_async_write(ucsi, UCSI_CONTROL, &command, sizeof(command));
}
