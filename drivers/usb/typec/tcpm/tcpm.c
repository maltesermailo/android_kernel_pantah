...
struct tcpm_port {
	...
	/* Set to true when VDM State Machine will have traffic soon. */
	bool vdm_sm_running;
				    ...
				    const __le32 *payload, int cnt)
{
	...
	if (PD_VDO_SVDM(p[0])) {
		 ...
		 *  - If no response to send, it will be cleared later.
		 *  - If there are responses to send, it will be cleared in the state machine.
		 * For the cases of RSP received:
		 *  - If no further INIT to send, it will be cleared later.
