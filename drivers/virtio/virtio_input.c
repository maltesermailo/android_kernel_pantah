...
static int virtinput_ff_upload(struct input_dev *dev, struct ff_effect *effect, struct ff_effect *old) {
  ...
  if (effect->type == FF_RUMBLE) {
    ...
    dev_info(&dev->dev, "virtinput_ff_upload FF_RUMBLE strong: %d, weak: %d",
             effect->u.rumble.strong_magnitude, effect->u.rumble.weak_magnitude);
    ...
    dev_info(&dev->dev, "virtinput upload ret: %d", ret);
    return ret;
  } else {
    dev_warn(&dev->dev, "virtinput_ff_upload did not receive effect type FF_RUMBLE: %d", effect->type);
  }
  return 0;
}
...
static int virtinput_ff_playback(struct input_dev *dev, int effect_id, int value) {
  ...
  dev_info(&dev->dev, "virtinput_ff_playback called with effect_id: %d, value: %d", effect_id, value);
  vi = input_get_drvdata(dev);

  ret = virtinput_send_status(vi, EV_FF, effect_id, value);
  dev_info(&dev->dev, "virtinput_ff_playback ret: %d", ret);
  return ret;
}
