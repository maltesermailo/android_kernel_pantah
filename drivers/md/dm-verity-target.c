...
#define DM_VERITY_INLINE_VERIFICATION_LIMIT_BYTES	8192
...
/* An accurately-named alias to DM_VERITY_OPT_TASKLET_VERIFY */
...
static unsigned int dm_verity_inline_verification_limit_bytes[4] = {
	DM_VERITY_INLINE_VERIFICATION_LIMIT_BYTES,	// IOPRIO_CLASS_NONE
	DM_VERITY_INLINE_VERIFICATION_LIMIT_BYTES,	// IOPRIO_CLASS_RT
	DM_VERITY_INLINE_VERIFICATION_LIMIT_BYTES,	// IOPRIO_CLASS_BE
	0				// IOPRIO_CLASS_IDLE
};

module_param_array_named(use_bh_bytes, dm_verity_inline_verification_limit_bytes, uint, NULL, 0644);

static DEFINE_STATIC_KEY_FALSE(verity_inline_verification_enabled);
			       ...
			       u8 *want_digest)
{
	...
	if (static_branch_unlikely(&verity_inline_verification_enabled) &&
	    io->verifying_inline) {
		data = dm_bufio_get(v->bufio, hash_block, &buf);
		if (data == NULL) {
			/*
			 * Inline verification and the hash was not in the bufio cache.
	...
	if (!aux->hash_verified) {
		...
		r = verity_hash(v, io, data, 1 << v->hash_dev_block_bits,
				io->tmp_digest, !io->verifying_inline);
		if (unlikely(r < 0))
			goto release_ret_r;
		...
		if (likely(memcmp(io->tmp_digest, want_digest,
				  v->digest_size) == 0))
			aux->hash_verified = 1;
		else if (static_branch_unlikely(&verity_inline_verification_enabled) &&
			 io->verifying_inline) {
			/*
			 * Error handling code (FEC included) cannot be run in
			 * an inline verification cotext since it may sleep,
					    ...
					    struct pending_block *block)
{
	...
	if (static_branch_unlikely(&verity_inline_verification_enabled) &&
	    io->verifying_inline) {
		/*
		 * Error handling code (FEC included) cannot be run inline
		 * so fallback to a standard workqueue.
		 */
		return -EAGAIN;
	}
...
static inline bool verity_should_verify_inline(unsigned int bytes, unsigned short ioprio)
{
	return ioprio <= IOPRIO_CLASS_IDLE &&
		bytes <= READ_ONCE(dm_verity_inline_verification_limit_bytes[ioprio]) &&
		!need_resched();
}
...
static void verity_end_io(struct bio *bio)
{
	...
	if (static_branch_unlikely(&verity_inline_verification_enabled) && io->v->verify_inline &&
...
static void verity_dtr(struct dm_target *ti)
{
	...
	if (v->verify_inline)
		static_branch_dec(&verity_inline_verification_enabled);
				 ...
				 bool only_modifier_opts)
{
	...
	do {
		...
		if (verity_is_verity_mode(arg_name)) {
			...
			static_branch_inc(&verity_inline_verification_enabled);
			continue;
...
static int verity_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	 ...
	 * "try_verify_inline" feature: WQ_HIGHPRI
