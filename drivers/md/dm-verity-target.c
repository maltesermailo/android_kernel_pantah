...
static unsigned int dm_verity_use_tasklet_blocks[4] = {
	DM_VERITY_USE_TASKLET_DEFAULT_BLOCKS,	// IOPRIO_CLASS_NONE
	DM_VERITY_USE_TASKLET_DEFAULT_BLOCKS,	// IOPRIO_CLASS_RT
	DM_VERITY_USE_TASKLET_DEFAULT_BLOCKS,	// IOPRIO_CLASS_BE
	0					// IOPRIO_CLASS_IDLE
};

module_param_array_named(use_tasklet_blocks, dm_verity_use_tasklet_blocks, uint, NULL, 0644);
...
static inline bool verity_use_tasklet(struct dm_verity_io *io, unsigned short ioprio)
{
	return in_softirq() && ioprio <= IOPRIO_CLASS_IDLE &&
		io->n_blocks <= READ_ONCE(dm_verity_use_tasklet_blocks[ioprio]);
}
...
static void verity_end_io(struct bio *bio)
{
	...
	if (static_branch_unlikely(&use_tasklet_enabled) && io->v->use_tasklet) {
		if (verity_use_tasklet(io, IOPRIO_PRIO_CLASS(bio->bi_ioprio))) {
			int err;
			io->in_tasklet = true;
