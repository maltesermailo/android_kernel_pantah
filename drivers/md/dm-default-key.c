...
struct default_key_c {
	...
	u8 raw_key[AES_256_XTS_KEY_SIZE];
};
...
static void default_key_dtr(struct dm_target *ti)
{
	...
	if (dkc->dev) {
		err = blk_crypto_evict_key(dkc->dev->bdev->bd_queue,
					   dkc->raw_key, dkc->crypto_mode,
					   dkc->sector_size);
		if (err && err != -ENOKEY)
...
static int default_key_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	if (strcmp(dkc->cipher_string, "aes-xts-plain64") != 0) {
		ti->error = "Unsupported encryption mode";
		err = -EINVAL;
		goto bad;
	}
	dkc->crypto_mode = BLK_ENCRYPTION_MODE_AES_256_XTS;

	/* <key> */
	key_size = strlen(argv[1]);
	if (key_size != 2 * AES_256_XTS_KEY_SIZE) {
		ti->error = "Unsupported key size";
		err = -EINVAL;
		goto bad;
	}
	key_size /= 2;
	if (hex2bin(dkc->raw_key, argv[1], key_size) != 0) {
	...
	err = blk_crypto_start_using_mode(dkc->crypto_mode, dkc->sector_size,
					  dkc->dev->bdev->bd_queue);
	if (err) {
		ti->error = "Error initializing blk-crypto";
		goto bad;
	}
...
static int default_key_map(struct dm_target *ti, struct bio *bio)
{
	 ...
	 */
	if ((bio->bi_opf & REQ_SKIP_DM_DEFAULT_KEY) || !bio_has_data(bio))
		return DM_MAPIO_REMAPPED;
	...
	dun = dkc->iv_offset + sector_in_target;
	if (dun & ((dkc->sector_size >> SECTOR_SHIFT) - 1))
		return DM_MAPIO_KILL;
	dun >>= dkc->sector_bits - SECTOR_SHIFT;

	if (bio_crypt_set_ctx(bio, dkc->raw_key, dkc->crypto_mode, dun,
			      dkc->sector_bits, GFP_NOIO) != 0)
		return DM_MAPIO_KILL;
			       ...
			       unsigned int maxlen)
{
	...
	case STATUSTYPE_TABLE:
		...
		num_feature_args += (dkc->sector_size != SECTOR_SIZE);
