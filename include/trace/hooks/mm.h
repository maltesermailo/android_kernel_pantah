...
DECLARE_HOOK(android_vh_should_alloc_pages_retry,
        ...
        TP_ARGS(gfp_mask, order, alloc_flags, migratetype, preferred_zone, page, should_alloc_retry));
...
DECLARE_HOOK(android_vh_rmqueue_bulk_bypass,
	TP_PROTO(unsigned int order, struct per_cpu_pages *pcp, int migratetype, struct list_head *list),
...
DECLARE_HOOK(android_vh_tune_mmap_readaround,
	TP_PROTO(struct readahead_control *ractl, pgoff_t pgoff),
	TP_ARGS(ractl, pgoff));
