...
DECLARE_HOOK(android_vh_binder_transaction_buffer,
...
DECLARE_HOOK(android_vh_binder_special_task,
	TP_PROTO(struct binder_transaction *t, struct binder_proc *proc,
		struct binder_thread *thread, struct binder_work *w, bool sync, bool *special_task),
	TP_ARGS(t, proc, thread, w, sync, special_task));
DECLARE_HOOK(android_vh_binder_enqueue_work_ilocked,
	TP_PROTO(struct binder_work *work, struct list_head *head),
	TP_ARGS(work, head));
