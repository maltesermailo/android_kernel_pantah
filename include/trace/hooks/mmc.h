...
DECLARE_RESTRICTED_HOOK(android_rvh_mmc_update_partition_status,
	TP_PROTO(struct mmc_card *card),
	TP_ARGS(card), 1);

DECLARE_RESTRICTED_HOOK(android_rvh_mmc_sd_update_cmdline_timing,
	TP_PROTO(struct mmc_card *card, int *err),
	TP_ARGS(card, err), 1);

DECLARE_RESTRICTED_HOOK(android_rvh_mmc_sd_update_dataline_timing,
	TP_PROTO(struct mmc_card *card, int *err),
	TP_ARGS(card, err), 1);
