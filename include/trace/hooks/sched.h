...
struct task_struct;
DECLARE_RESTRICTED_HOOK(android_rvh_select_task_rq,
	TP_PROTO(struct task_struct *p, int prev_cpu, int sd_flags,
			int wake_flags, int *target_cpu),
	TP_ARGS(p, prev_cpu, sd_flags, wake_flags, target_cpu), 1);
...
#define trace_android_rvh_select_task_rq(p, prev_cpu, sd_flags, wake_flags, target_cpu)
